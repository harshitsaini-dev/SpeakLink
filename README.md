...
directory. It does not use `backend/speaklink_live.db`.
...
explicit `SPEAKLINK_TEST_BASE_URL`, explicit confirmation that the target server
...
`SPEAKLINK_ALLOW_NONLOCAL_WRITE_TESTS=1` is deliberately set. Do not point that
...
rename, reset, or reseed `backend/speaklink_live.db` to make tests pass. Automated
