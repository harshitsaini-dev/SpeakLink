...
Legacy UUID-hex and `echocast_rcv` formats remain strictly separated. Dual
...
playback confirmation, EchoGuard verification, or audible speaker output.
...
configuration changed, and `backend/echocast_live.db` was not opened, copied,
...
- `backend/echocast_live.db` is refused **before any connection is opened**,
...
`scripts/Start-EchoCastLocalPilot.ps1` / `scripts/Stop-EchoCastLocalPilot.ps1`
wrap manual browser testing.

Isolated pilot design, all outside Git:

```text
%LOCALAPPDATA%\EchoCast-AI\local-pilot\
    data\echocast_local_pilot.db      disposable pilot database
...
- The protected `backend/echocast_live.db` is refused by exact path, by a
...
duplicated: `ECHOCAST_DB_PATH` (backend/db.py), `ADMIN_USERNAME`/
...
- Pilot database `%LOCALAPPDATA%\EchoCast-AI\local-pilot\data\echocast_local_pilot.db`
...
device, amplifier routing, EchoGuard, and acoustic verification. None of these
exist yet.
...
amplifier or Bluetooth control, no EchoGuard, no acoustic verification, no
...
Configuration is process-scoped via `ECHOCAST_AUDIO_SINK_MODE`
(`null` default | `windows`) and `ECHOCAST_AUDIO_OUTPUT_DEVICE`.
...
Implement **EchoGuard acoustic verification for one wired Store pilot**: an
...
`backend/echocast_live.db` plus any WAL/SHM files to a path outside the
...
`backend/echocast_live.db` was never opened, copied or modified: 507 904 bytes,
...
9. EchoGuard pause/resume integration does not exist.
...
isolated `ECHOCAST_DB_PATH`. That assertion is a real safety guard — if another
...
`test_smoke.py`, and additionally sets a disposable `ECHOCAST_DB_PATH` before
...
- Enrolment HTTP endpoints, and Windows Receiver agent enrolment with the
  credential sealed by DPAPI under `%LOCALAPPDATA%\EchoCast-AI\receiver`.
...
1. **The Receiver discards audio by default.** `resolve_sink_configuration`
   returns the *null* sink unless `ECHOCAST_AUDIO_SINK_MODE` says otherwise
...
3. **The escape hatch was unreachable and failed like a crash.** Windows mode
   ...
   attempted `ECHOCAST_AUDIO_OUTPUT_DEVICE="Speakers (Realtek(R) Audio)"` did —
...
| consistent backup | `backups/echocast_live-20260729-160359.db` (SQLite backup API) |
...
`EchoCastReceiverBackground.exe` is GUI-subsystem and has no console.
...
`Install-EchoCastReceiverLanPilot.ps1` registers `EchoCastReceiver.exe`, so a
Store still carrying `EchoCast Receiver LAN Pilot (disposable)` shows a window at
every logon regardless of the FFmpeg fix — and both tasks would compete for the
same credential. The Store installer now removes obsolete EchoCast tasks,
...
9. **Windows service / boot-before-logon operation.** Not a missing flag: the
   ...
   built so far.
10. EchoGuard pause/resume against actual EchoGuard.
...
`locate_verified_receiver_package()` finds the newest `EchoCastReceiver-*`
package `Build-EchoCastReceiver.ps1` already produces under `artifacts/`, and
...
`Test-EchoCastReceiverPackage.ps1`, which stays the authoritative build-time
gate.

### A second defect, found only by actually building the package

`Build-EchoCastReceiver.ps1` had apparently never been run end to end on this
...
```
EchoCastReceiver-1.0.0-7e6d704-20260730-124134
  Test-EchoCastReceiverPackage.ps1   ECHOCAST_RECEIVER_PACKAGE_VERIFIED (20+ checks)
...
Every real-PostgreSQL test runs inside a generated `echocast_test_*`
...
The leaked tables were dropped by name (EchoCast-owned tables only) and
...
instead by the project-ref fingerprint plus zero EchoCast statements in an
...
Source: a fresh SQLite backup of live RC12 (`echocast-20260801-pre-supabase-
...
## Incident: EchoCast Live outage, and the three supervisor defects behind it
...
`EchoCastHQ-0.1.0-rc16-227c2b8-20260802-180703`, 31/31 PASS, installed.
...
| | |
|---|---|
| ZIP | `artifacts/EchoCast-Store-Kit-1.3.0-6ff3fb2-20260805-073937.zip` |
...
| Rollback kit | `EchoCast-Store-Kit-1.2.0-1f4727c-20260803-085407.zip` **retained, untouched** |

Builder audit passed: no credential/key/database/log/developer path, EchoCast
...
worker, `EchoCast Live startup complete`, Receiver key *"reused unchanged"*.

| | |
|---|---|
| Rollback backup | `_live-hq-deployment-backups/echocast-before-runtime-disconnect-20260805-154400.db` |
...
This deliberately reverses the earlier design. Scaling EchoCast's own PCM meant
...
* **Other applications on the same endpoint are affected.** Chrome, EchoGuard,
  till audio — anything sharing that output gets louder, quieter or muted with
  EchoCast. No per-app isolation in this release.
...
way to silence EchoCast without touching a shared endpoint — but nothing moves
...
| | |
|---|---|
| ZIP | `artifacts/EchoCast-Store-Kit-1.5.0-f363b1f-20260805-135058.zip` |
...
| | |
|---|---|
| Rollback backup | `_live-hq-deployment-backups/echocast-before-two-way-sync-20260806-114500.db` |
...
the stale `backend/echocast_live.db` on the default path holds a July snapshot
of 13 demo Stores. The live database is `data/echocast.db` (44 Stores), which
...
and `EchoCast Live startup complete`.
...
of an active broadcast.** With nothing on air EchoCast does not observe or
...
somebody had moved the slider. After STOP, EchoCast leaves the mixer alone.
...
| | |
|---|---|
| ZIP | `artifacts/EchoCast-Store-Kit-1.7.0-0f2491c-20260806-115716.zip` |
...
| | |
|---|---|
| ZIP | `artifacts/EchoCast-Store-Kit-1.8.0-81aa464-20260808-083245.zip` |
...
endpoint stays under EchoCast control, and the mixer is not left alone.
