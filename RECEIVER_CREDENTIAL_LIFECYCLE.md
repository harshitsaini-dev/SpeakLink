...
`backend/echocast_live.db` path before connection and does not read keys from
environment variables or persist them.

The rehearsal fixtures use generated test-only key version 1 for backfilled
`legacy_uuid_hex` HMAC rows and version 2 for newly enrolled `echocast_rcv`
...
READY, audio receipt, playback, EchoGuard verification, or audible speakers.
