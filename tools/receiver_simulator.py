"""Local non-audio SpeakLink receiver protocol simulator.
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--token",
        help="Receiver credential; defaults to SPEAKLINK_RECEIVER_TOKEN",
    )
...
async def _run_from_arguments(arguments: argparse.Namespace) -> ScenarioResult:
    token = arguments.token or os.environ.get("SPEAKLINK_RECEIVER_TOKEN")
    if not token:
        raise SimulatorConfigurationError(
            "receiver credential is required via --token or SPEAKLINK_RECEIVER_TOKEN"
        )
