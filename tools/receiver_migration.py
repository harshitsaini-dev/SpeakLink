...
PROTECTED_DATABASE = (BACKEND_ROOT / "speaklink_live.db").resolve()
...
class ProtectedDatabaseRefused(MigrationToolError):
    """Someone pointed this at backend/speaklink_live.db."""
...
def _resolved(path) -> Path:
    ...
    if resolved == PROTECTED_DATABASE:
        raise ProtectedDatabaseRefused(
            "that is the protected SpeakLink database. This tool will not touch it. "
