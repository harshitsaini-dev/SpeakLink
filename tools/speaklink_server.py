"""Repo-native SpeakLink HQ launcher. One command, any operating system.
...
#: job with C:\Windows\System32 as its cwd, and `python tools/speaklink_server.py`
...
DATA_DIR_ENV = "SPEAKLINK_DATA_DIR"
...
DB_PATH_ENV = "SPEAKLINK_DB_PATH"                 # backend/db_config.py
KEY_CONTAINER_ENV = "SPEAKLINK_KEY_CONTAINER"     # backend/receiver_key_bootstrap.py
FRONTEND_BUILD_ENV = "SPEAKLINK_FRONTEND_BUILD"   # backend/server.py
...
def command_run(config: Config) -> int:
    ...
    if existing:
        raise LaunchError(
            f"SpeakLink HQ is already running in this repository (pid {existing}).\n"
            "  Use restart, or stop it first.")

    print(f"SpeakLink HQ starting on {config.display_url}")
...
def command_start(config: Config) -> int:
    ...
    existing = running_pid()
    if existing:
        print(f"SpeakLink HQ is already running in this repository (pid {existing}).")
    ...
    print(f"SpeakLink HQ starting (pid {process.pid})")
    ...
    if process.poll() is not None:
        clear_runtime_files()
        raise LaunchError(
            f"SpeakLink HQ exited during startup (code {process.returncode}).\n"
            f"  Read the log: {log_path}")
    raise LaunchError(
        f"SpeakLink HQ did not answer within {STARTUP_TIMEOUT_SECONDS}s.\n"
...
def command_stop(config: Config) -> int:
    ...
    if pid is None:
        if state_file().is_file() or pid_file().is_file():
            clear_runtime_files()
            print("SpeakLink HQ was not running. Cleared a stale runtime file.")
        else:
            print("SpeakLink HQ is not running in this repository.")
        return 0

    print(f"Stopping SpeakLink HQ (pid {pid})")
    if platform.system() == "Windows":
    ...
    raise LaunchError(f"SpeakLink HQ (pid {pid}) did not stop within "
...
def command_status(config: Config) -> int:
    ...
    print("SpeakLink HQ (repo-native)")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="speaklink_server",
        description="Run SpeakLink HQ from this repository. No installer, no "
