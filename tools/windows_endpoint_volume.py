...
WHAT THIS CHANGES ABOUT ECHOCAST
...
rather than discovered later:
  ...
  * every other application on that endpoint is affected. Chrome, EchoGuard,
    anything sharing the output gets louder, quieter or muted with EchoCast.
...
new USB headset can point EchoCast at the wrong output and mute a shop's till
...
class EndpointControlUnavailable(EndpointControlError):
    ...
    Distinct from "the endpoint is missing": one means EchoCast cannot control
...
class EndpointNotFound(EndpointControlError):
    ...
    def __init__(self, endpoint_id: str) -> None:
        super().__init__(
            "The Windows audio output EchoCast was configured to control is no "
...
def resolve_endpoint_for_playback_device(playback_name: str, *,
    ...
    raise EndpointAmbiguous(
        ...
        "EchoCast cannot tell which one to control. Rename one of them in "
