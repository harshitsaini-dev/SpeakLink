"""Local one-Store pilot readiness harness for SpeakLink.
...
What it does NOT prove:
    ...
    amplifier behaviour, audible Store speakers, LinkGuard, or production
...
PROTECTED_DATABASE_PATH = BACKEND_DIR / "speaklink_live.db"

PILOT_MARKER_NAME = "PILOT_ONLY"
PILOT_DATABASE_NAME = "speaklink_local_pilot.db"

# Environment variable names proven from the current application code.
DB_PATH_ENV = "SPEAKLINK_DB_PATH"          # backend/db.py
...
def default_pilot_root() -> Path:
    """Resolve %LOCALAPPDATA%\\SpeakLink\\local-pilot without hardcoding a user."""
    base = os.environ.get("LOCALAPPDATA")
    if not base:
        base = os.environ.get("XDG_STATE_HOME") or str(Path.home() / ".local" / "state")
    return Path(base).expanduser().resolve() / "SpeakLink" / "local-pilot"
...
def reject_protected_database(candidate: str | os.PathLike[str]) -> Path:
    ...
    if resolved == protected:
        raise ProtectedPathError(
            "the protected SpeakLink database was refused; the pilot uses its own "
    ...
    try:
        if resolved.exists() and protected.exists() and os.path.samefile(resolved, protected):
            raise ProtectedPathError(
                "the supplied path is the protected SpeakLink database; refused"
            )
...
def _ensure_layout(paths: PilotPaths) -> None:
    ...
    if not paths.marker_path.exists():
        paths.marker_path.write_text(
            "SpeakLink local pilot directory.\n"
...
import os, sys
from pathlib import Path
expected = Path(os.environ["SPEAKLINK_DB_PATH"]).resolve()
import db
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--pilot-root",
        default=None,
        help="Pilot root directory. Defaults to %%LOCALAPPDATA%%\\SpeakLink\\local-pilot.",
    )
    parser.add_argument(
        "--reset-pilot-db",
        action="store_true",
