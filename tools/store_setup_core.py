...
_FORBIDDEN_TEXT_PATTERNS = (
    "speaklink_rcv_v", "BEGIN PRIVATE KEY", "BEGIN RSA PRIVATE KEY",
...
def verify_receiver_package(package_path) -> PackageVerdict:
    ...
    Test-SpeakLinkReceiverPackage.ps1, which remains the authoritative build-time
    ...
    console_exe = package_path / "SpeakLinkReceiver.exe"
    background_exe = package_path / "SpeakLinkReceiverBackground.exe"
    ...
    if background_exe.exists():
        try:
            if read_pe_subsystem(background_exe) != 2:
                reasons.append("SpeakLinkReceiverBackground.exe is not WINDOWS_GUI")
        except (OSError, ValueError):
            reasons.append("could not read SpeakLinkReceiverBackground.exe's PE header")
    if console_exe.exists():
        try:
            if read_pe_subsystem(console_exe) != 3:
                reasons.append("SpeakLinkReceiver.exe is not WINDOWS_CUI")
        except (OSError, ValueError):
            reasons.append("could not read SpeakLinkReceiver.exe's PE header")
...
def locate_verified_receiver_package(artifacts_root=None) -> Path:
    """The newest SpeakLinkReceiver-* package under artifacts/ that verifies.

    Never a hardcoded 'artifacts/receiver-package' placeholder: package
    directories are versioned and timestamped
    (SpeakLinkReceiver-<version>-<commit>-<timestamp>), sort lexicographically
    ...
    if not root.exists():
        raise NoVerifiedReceiverPackage(
            ...
            "with Build-SpeakLinkReceiver.ps1 first."
        )
    candidates = sorted(
        (item for item in root.iterdir() if item.is_dir() and item.name.startswith("SpeakLinkReceiver-")),
        key=lambda p: p.stat().st_mtime, reverse=True,
    )
    if not candidates:
        raise NoVerifiedReceiverPackage(
            f"no SpeakLinkReceiver-* package exists under {root}. Build one with "
            "Build-SpeakLinkReceiver.ps1 first."
        )
...
# ===========================================================================
DEFAULT_TASK_NAME = "SpeakLink Store Receiver"
...
_FORBIDDEN_EXPORT_PATTERNS = ("speaklink_rcv_v", "Bearer ", "BEGIN PRIVATE KEY",
