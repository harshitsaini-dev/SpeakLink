"""Where a packaged EchoCast application finds the files it ships with.
...
``_internal\\scripts\\Manage-EchoCastStoreReceiverTask.ps1``, found neither, and
...
``ECHOCAST_RESOURCE_ROOT`` overrides all of it, which is what lets the tests drive
...
RESOURCE_ROOT_ENV = "ECHOCAST_RESOURCE_ROOT"
...
def resolve(*parts: str, required: bool = True) -> Path:
    ...
    if required and not candidate.exists():
        raise ResourceNotFound(
            ...
            "with scripts/Build-EchoCastStoreSetupPackage.ps1, which is what puts "
...
def resolve_packaged_ffmpeg(*, allow_path_fallback=None):
    ...
    if packaged:
        raise PackagedFfmpegMissing(
            f"the bundled FFmpeg is missing from {candidate}. This EchoCast setup "
