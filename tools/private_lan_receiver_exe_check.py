...
WHAT IT IS NOT EVIDENCE OF
    ...
    A second desktop. An amplifier. An audible speaker. LinkGuard.
...
TASK_NAME = "SpeakLink LAN Check (disposable)"

PROTECTED_DB = ROOT / "backend" / "speaklink_live.db"
...
SECRET_PATTERNS = [
    re.compile(r"speaklink_rcv_v1\.[A-Za-z0-9._\-]+"),
    ...
    while not watching.is_set():
        ...
        try:
            ...
            for row in rows:
                ...
                if row["name"].lower() == "speaklinkreceiver.exe" and row["cmdline"]:
...
# ===========================================================================
root = Path(os.environ["TEMP"]) / f"speaklink-lan-check-{time.strftime('%H%M%S')}"
...
try:
    # ---- 1. firewall -----------------------------------------------------
    print("=== checks ===")
    firewall = powershell("-File", str(ROOT / "scripts" / "Test-SpeakLinkLanPilotFirewall.ps1"), timeout=300)
    record("01_firewall_verified", "SPEAKLINK_LAN_PILOT_FIREWALL_VERIFIED" in firewall.stdout)
    ...
    record("06_speaklink_live_title_loads", "<title>SpeakLink</title>" in index)
    ...
    install = powershell(
        "-File", str(INSTALLER / "Install-SpeakLinkReceiverLanPilot.ps1"),
    ...
    record("31_task_installs_from_the_kit",
           "SPEAKLINK_RECEIVER_TASK_INSTALLED" in install.stdout)

    verify = powershell(
        "-File", str(INSTALLER / "Test-SpeakLinkReceiverLanPilot.ps1"),
        "-TaskName", TASK_NAME, timeout=600)
    record("32_current_user_at_logon_task",
           "SPEAKLINK_RECEIVER_TASK_SCHEDULER_VERIFIED" in verify.stdout)
    ...
    powershell("-File", str(INSTALLER / "Install-SpeakLinkReceiverLanPilot.ps1"),
    ...
    powershell("-File", str(ROOT / "scripts" / "Uninstall-SpeakLinkReceiverLanPilot.ps1"),
    ...
    for process in (frontend, backend):
        if process and process.poll() is None:
            ...
            try:
                ...
                process.kill()
    subprocess.run(["taskkill", "/F", "/IM", "SpeakLinkReceiver.exe"],
...
leftover = subprocess.run(
    ["powershell.exe", "-NoProfile", "-Command",
     "@(Get-Process SpeakLinkReceiver,ffmpeg -ErrorAction SilentlyContinue).Count"],
    capture_output=True, text=True)
...
print()
if failing:
    print("SPEAKLINK_PRIVATE_LAN_RECEIVER_EXE_CHECK_FAILED")
    print("  failing:", failing)
    raise SystemExit(1)

print(f"{len(checks)} checks passed")
print()
print("SPEAKLINK_PRIVATE_LAN_RECEIVER_EXE_CHECK_PASSED")
print()
...
print("LinkGuard, or SPEAKER_VERIFIED.")
