...
The security audit found the live ``ADMIN`` password inside ``speaklink-live.zip``
...
def resolve_target(target: str) -> Path:
    ...
    if target == TARGET_PROTECTED:
        return REPOSITORY_ROOT / "backend" / "speaklink_live.db"
...
# ===========================================================================
def running_speaklink_processes() -> "list[str]":
    ...
    """
    script = (
        "$names = @('SpeakLinkHQRuntime','python','pythonw'); "
        "Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | "
        "Where-Object { $names -contains ($_.Name -replace '\\.exe$','') } | "
        "Where-Object { $_.CommandLine -match 'uvicorn|SpeakLinkHQRuntime|server:app' } | "
...
def main(argv=None) -> int:
    ...
    if arguments.target == TARGET_PERSISTENT:
        ...
        backups = Path.home() / "speaklink-database-backups"
    ...
    describe(result)
    print()
    print("SPEAKLINK_HQ_PASSWORD_CHANGED")
