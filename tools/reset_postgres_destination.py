"""Empty the EchoCast tables in a PostgreSQL destination, and nothing else.
...
* it knows the EchoCast table inventory by name and will touch nothing else;
...
#: Every table EchoCast owns, in the order the migration tool populates them
...
#: these; damaging one breaks the project itself, not just EchoCast's data.
...
def inspect_destination(engine, *, schema: str = DEFAULT_SCHEMA
    ...
    * ``counts``   - row count per EchoCast table that exists
    * ``missing``  - EchoCast tables not present (fine: a fresh project)
    * ``unexpected`` - tables this tool does not recognise (NOT fine)
    """
    from sqlalchemy import inspect, text

    known = _echocast_tables()
...
def reset(engine, *, counts: dict[str, int], schema: str = DEFAULT_SCHEMA
    ...
    """DELETE every row from the EchoCast tables, children first.
    ...
    with engine.begin() as connection:
        ...
        for table in reversed(_echocast_tables()):
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("=== EchoCast PostgreSQL destination reset ===")
    ...
    print()
    print("  EchoCast tables carrying rows:")
    ...
    print(f"  EchoCast tables present but empty : {sum(1 for n in counts.values() if not n)}")
    print(f"  EchoCast tables not present yet   : {len(missing)}"
    ...
    if unexpected:
        ...
        for table in unexpected:
            print(f"    {table}")
        print("REFUSED: this destination holds public tables EchoCast does not own. "
    ...
    print(f"  EchoCast rows remaining : {sum(after.values())}")
    ...
    print()
    print("ECHOCAST_DESTINATION_RESET_COMPLETE")
    return 0
