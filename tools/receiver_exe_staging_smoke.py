...
Run it with the package directory as the only argument:
    ...
    python tools/receiver_exe_staging_smoke.py artifacts/EchoCastReceiver-<...>
...
ROOT = Path(r"c:\Users\admin\Desktop\EchoCast-AI\HQ-Broadcast-Full (1)")
...
root = Path(os.environ["TEMP"]) / f"echocast-exe-{time.strftime('%H%M%S')}"
...
try:
    ...
    if all(checks.values()):
        print("ECHOCAST_RECEIVER_EXE_STAGING_SMOKE_PASSED")
    else:
        print("ECHOCAST_RECEIVER_EXE_STAGING_SMOKE_FAILED")
