...
that already knows how to do its job:
    ...
    scripts/Build-EchoCastReceiver.ps1          the Receiver, with FFmpeg
    scripts/Test-EchoCastReceiverPackage.ps1    verifies that package
    scripts/Build-EchoCastStoreSetupPackage.ps1 wraps it with the wizard,
...
FORBIDDEN_CONTENT = (
    rb"echocast_rcv_v1\.[A-Za-z0-9._\-]+",
    rb"ECHO(-[A-Z0-9]{4}){2,}",
    rb"C:\\Users\\admin",
)
...
EXPECTED_EXECUTABLES = (
    ...
    "EchoCastReceiverBackground.exe",
)

CANONICAL_ICON = REPO_ROOT / "assets" / "echocast.ico"
...
def audit_tree(root: Path) -> list[str]:
    ...
    # Every EchoCast executable must carry the EchoCast icon, and the
    ...
    for executable in sorted(root.rglob("*.exe")):
        ...
        if biggest_icon not in raw:
            findings.append(f"missing the EchoCast icon: {relative}")
        if executable.name == "EchoCastReceiverBackground.exe":
            if pe_subsystem(executable) != 2:
                findings.append(
                    f"{relative} is a console application - it would put a "
                    "black window on a Store counter")
        if executable.name == "EchoCastStoreSetup.exe":
            if pe_subsystem(executable) != 2:
...
def build(receiver_version: str, setup_version: str, ffmpeg: str | None,
    ...
    if skip_build:
        ...
        receiver_package = newest_package("EchoCastReceiver")
        setup_package = newest_package("EchoCastStoreSetup")
    else:
        print("=== 1/3  building the Receiver ===")
        arguments = ["-Version", receiver_version]
        if ffmpeg:
            arguments += ["-FfmpegPath", ffmpeg]
        run_powershell(SCRIPTS / "Build-EchoCastReceiver.ps1", arguments)
        receiver_package = newest_package("EchoCastReceiver")

        print("=== 2/3  verifying the Receiver package ===")
        run_powershell(SCRIPTS / "Test-EchoCastReceiverPackage.ps1",
                       ["-PackagePath", str(receiver_package),
                        "-Version", receiver_version])

        print("=== 3/3  building the Store Setup package ===")
        run_powershell(SCRIPTS / "Build-EchoCastStoreSetupPackage.ps1",
    ...
    print("  every EchoCast executable carries the EchoCast icon        PASS")
    ...
    zip_name = f"EchoCast-Store-Kit-{setup_version}-{git_commit()}-{stamp}.zip"
