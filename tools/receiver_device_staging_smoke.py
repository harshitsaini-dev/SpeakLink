...
Safety: a fresh temporary database every run. ``backend/speaklink_live.db`` and
...
def _reject_protected_paths(staging_root: Path) -> None:
    ...
    pilot_root = Path(os.environ.get("SPEAKLINK_PILOT_ROOT", "")).expanduser()
    if str(pilot_root):
...
def smoke(staging_root: Path) -> dict:
    ...
    environment.update({
        ...
        "SPEAKLINK_KEY_CONTAINER": str(key_container),
        "SPEAKLINK_KEY_PROTECTOR": "fake",
        "JWT_SECRET": jwt_secret,
        "ADMIN_USERNAME": admin_username,
        "ADMIN_PASSWORD": admin_password,
        "CORS_ORIGINS": "http://localhost:3000",
        "SPEAKLINK_SEED_STORES": "0",
    })
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--staging-root",
        default=str(Path(tempfile.gettempdir()) / "speaklink-device-staging"),
    ...
    if report["overall_result"] == MARKER_PASSED:
        ...
        print("SPEAKER_VERIFIED. Only LinkGuard acoustic detection can establish those.")
        return EXIT_OK
