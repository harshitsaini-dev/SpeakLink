...
```
EchoCast-Store-Pilot-<commit>-<timestamp>/
  Receiver/          EchoCastReceiver.exe, ffmpeg.exe, _internal/,
                     manifest.json, SHA256SUMS.txt, licenses/
  Installer/         Install- / Test- / Uninstall-EchoCastReceiverLanPilot.ps1
...
```powershell
.\scripts\Build-EchoCastStorePilotKit.ps1 `
...
Ends with `ECHOCAST_STORE_PILOT_KIT_BUILT`.
...
37 checks. Ends with `ECHOCAST_STORE_PILOT_KIT_VERIFIED` and exit code `0`.
...
amplifier, an audible speaker, EchoGuard, or `SPEAKER_VERIFIED`. Verifying a kit
