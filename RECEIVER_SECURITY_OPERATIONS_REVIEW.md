# SpeakLink Receiver Security and Operations Review

Status: review-only; no production authorization. This review does not select, generate, load, or configure a real secret, and does not open, copy, back up, or modify `backend/speaklink_live.db`.
...
Production deployment decisions are not yet implemented. No real migration or hashed cutover has happened against `backend/speaklink_live.db`. No production cutover has occurred. No real HMAC key was loaded. No real database was opened, copied, or modified as part of this review or any prior task in this line of work. The project is not yet proven production-ready for an all-40-Store rollout: only one Uvicorn worker has been exercised, connection state is process-local, and no Windows Receiver Agent, FFmpeg output path, amplifier, or LinkGuard acoustic verification has been exercised end to end.
...
- SQLite database (`backend/speaklink_live.db`).
...
- LinkGuard evidence (`SPEAKER_VERIFIED`; separate trusted-verifier path, not yet integrated).
...
- SpeakLink to LinkGuard control: reserved trusted-verifier schema only; no transport exists yet.
- Store speakers to LinkGuard microphone: acoustic boundary, entirely outside this codebase's current scope.
...
| 22 | SQLite database | Accidental real-database testing | Protected-path guards refuse `speaklink_live.db` in every isolated module | None needed beyond current guards | Low | Low | Keep protected-path guards in all future services | No | `migrations.py`, backfill/auth/transition modules |
...
The future SpeakLink service identity should need only:
