# EchoCast One-Store Live-Audio Software Test Runbook
...
database. The protected application database `backend\echocast_live.db` is
...
4. No EchoCast process already running:
   ```powershell
...
```powershell
.\scripts\Run-EchoCastAudioSmoke.ps1
```

Or directly:

```powershell
Set-Location 'C:\Users\admin\Desktop\EchoCast-AI\HQ-Broadcast-Full (1)\backend'
...
.\scripts\Start-EchoCastAudioReceiverPilot.ps1
```
...
```powershell
.\scripts\Stop-EchoCastAudioReceiverPilot.ps1
...
| `ECHOCAST_RECEIVER_TOKEN is not set` | The Receiver needs the Store credential in its environment. |
...
| `the protected EchoCast database was refused` | A path resolved to the real database. The safety net worked. |
...
Never delete the repository, and never delete `backend\echocast_live.db` or its
`-wal`/`-shm` files.
...
`SPEAKER_VERIFIED` requires EchoGuard acoustic detection, which is not part of
