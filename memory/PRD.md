# SpeakLink — HQ-to-Store Live Announcement Broadcasting

## Original Problem Statement
Build a separate, independent system that lets HQ operators speak live into a browser microphone and have that voice play in selected store speakers (like an internal FM radio broadcast). MUST NOT touch or depend on any existing LinkGuard AI system.

## Architecture
- **Backend**: FastAPI + SQLAlchemy + SQLite (speaklink_live.db), native WebSockets
...
- Configure `/app/backend/.env` (JWT_SECRET, ADMIN_USERNAME, ADMIN_PASSWORD, SPEAKLINK_DB_PATH)
...
- Compliance / audio-fingerprinting integration (belongs to LinkGuard AI)

## Files (do not modify LinkGuard AI — none present in this repo)
