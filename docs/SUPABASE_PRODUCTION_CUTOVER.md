...
| Database | local SQLite (`backend/speaklink_live.db` by default) | PostgreSQL (Supabase) |
...
```
%LOCALAPPDATA%\SpeakLink\persistent-lan-server\keys\database-url.txt
```
...
3. Stop the `SpeakLink HQ Runtime` Scheduled Task.
...
1. Stop the `SpeakLink HQ Runtime` Scheduled Task.
...
SpeakLink tables in a destination that already holds an older snapshot, so the
