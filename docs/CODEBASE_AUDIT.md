...
* **Evidence** — `scripts/Start-SpeakLinkLanPilot.ps1`
...
* `SpeakLinkHQRuntime.exe`, `SpeakLinkStoreSetup.exe` — **not built**
...
`scripts/Start-SpeakLinkLanPilot.ps1` · `tools/lan_pilot.py` ·
`backend/tests/test_pilot_scripts.py` · `backend/tests/test_protected_database_isolation.py` ·
every `.db` under the repository and `%LOCALAPPDATA%\SpeakLink` (read-only) ·
...
* `scripts/Initialize-SpeakLinkPersistentLanServer.ps1`
