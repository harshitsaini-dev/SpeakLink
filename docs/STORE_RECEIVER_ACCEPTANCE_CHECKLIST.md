...
```powershell
& "$env:LOCALAPPDATA\SpeakLink\receiver-app\SpeakLinkReceiver.exe" diagnose
```
...
3. Wait two minutes. On the Store PC check the Receiver is **still running**:
   ```powershell
   Get-Process SpeakLinkReceiverBackground -ErrorAction SilentlyContinue
   ```
...
1. End the Receiver deliberately:
   ```powershell
   Stop-Process -Name SpeakLinkReceiverBackground -Force
   ```
...
> `Repair-SpeakLinkStoreReceiver.ps1 -PackagePath .\Receiver -AudioOutputDevice "index:N@Name"`.
...
- **Audio before anybody logs in.** Not supported, not implemented, and not
  ...
  user's session.
- **`SPEAKER_VERIFIED`.** That is an LinkGuard hardware result. What you are
