...
`SpeakLinkHQRuntime.exe` is one windowed Windows process that starts the SpeakLink
...
database, use `backend/speaklink_live.db`, pick up a pilot file, start the
...
`%LOCALAPPDATA%\SpeakLink\hq-runtime-status.json` — machine-level, **not**
...
|---|---|---|
| No persistent root | **Refuse**, name `Initialize-SpeakLinkPersistentLanServer.ps1` | A runtime that can create a server will one day create one over the real one |
...
| Signing secret missing | **Create it** | Costs everybody one sign-in, not 44 re-enrolments. `Start-SpeakLinkPersistentLanServer.ps1` already mints it here |
...
- `sys.executable` inside a frozen build is `SpeakLinkHQRuntime.exe` itself — so
