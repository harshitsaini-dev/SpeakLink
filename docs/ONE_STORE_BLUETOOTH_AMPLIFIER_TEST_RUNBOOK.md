...
- Never touch `backend\speaklink_live.db`. Every step here uses the isolated
  pilot database under `%LOCALAPPDATA%\SpeakLink\local-pilot\`.
...
```powershell
.\scripts\Test-SpeakLinkAudioOutput.ps1 -Seconds 4
```
...
$env:SPEAKLINK_AUDIO_OUTPUT_DEVICE = 'index:4@Headphones (Bluetooth Stereo)'   # your verified selector
$env:SPEAKLINK_RECEIVER_TOKEN      = '<paste, never echo>'

.\scripts\Start-SpeakLinkWindowsAudioReceiverPilot.ps1
```
...
```powershell
$db = Join-Path $env:LOCALAPPDATA 'SpeakLink\local-pilot\data\speaklink_local_pilot.db'
...
```
%LOCALAPPDATA%\SpeakLink\local-pilot\logs\windows-audio-receiver-report.json
```
...
```powershell
.\scripts\Stop-SpeakLinkWindowsAudioReceiverPilot.ps1
...
Test-Path backend\speaklink_live.db-wal
Test-Path backend\speaklink_live.db-shm
```
...
.\scripts\Run-SpeakLinkAudioSmoke.ps1 -PilotRoot <a scratch directory>
