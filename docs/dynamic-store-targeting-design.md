...
| `ACTIVE` | EchoCast controls the endpoint; telemetry reports actual state; no enforcement |
| `PAUSING` | Restore this generation's baseline → **clear the record** → stop the observer |
| `PAUSED` | EchoCast does not touch the mixer and does not report it as controlled |
...
- **Power fails while PAUSED** → no record (cleared at the pause) → nothing to
  restore. Correct: EchoCast was not controlling the mixer.
...
| ACTIVE | EchoCast level | present | restore + clear | target STOPPED, session ended if orphaned | released | no |
...
3. **`stood_down` with `endpoint_restored: false`** leaves a Store audibly quiet
   but at EchoCast's level until its Receiver restarts. The design surfaces this
