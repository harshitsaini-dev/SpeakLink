...
starts `EchoCastHQRuntime.exe`.

| Script | What it does |
|---|---|
| [`Install-EchoCastHQAutoStart.ps1`](../scripts/Install-EchoCastHQAutoStart.ps1) | Verify a package, install it, register the task. Does **not** start it unless `-StartNow` |
| [`Test-EchoCastHQAutoStart.ps1`](../scripts/Test-EchoCastHQAutoStart.ps1) | Read-only verification, 30+ checks |
| [`Repair-EchoCastHQAutoStart.ps1`](../scripts/Repair-EchoCastHQAutoStart.ps1) | Replace broken application files, re-register a wrong task. Never touches data |
| [`Uninstall-EchoCastHQAutoStart.ps1`](../scripts/Uninstall-EchoCastHQAutoStart.ps1) | Remove task and application files. Keeps data |
...
| Setting | Value | Why |
|---|---|---|
| Action | `<InstallRoot>\EchoCastHQRuntime.exe`, **no arguments** | Everything configurable lives in the persistent profile, so the task cannot go stale and has nowhere to carry a secret |
...
```powershell
.\Install-EchoCastHQAutoStart.ps1 -PackagePath . -DryRun     # check everything
.\Install-EchoCastHQAutoStart.ps1 -PackagePath .             # register
Start-ScheduledTask -TaskName "EchoCast HQ Runtime"          # start deliberately
```
...
- a persistent root with no `data\echocast.db` or no `keys` folder
...
Emits `ECHOCAST_HQ_AUTO_START_VERIFIED`.
...
```powershell
.\Repair-EchoCastHQAutoStart.ps1 -PackagePath . -DryRun
```
...
- Removes the task **only** if its action runs `EchoCastHQRuntime.exe`
