...
1. Note the package you are running now:
   ...
   ```powershell
   Get-Content "$env:LOCALAPPDATA\SpeakLink\hq-app\manifest.json" |
...
```powershell
Stop-ScheduledTask -TaskName "SpeakLink HQ Runtime"
```

Confirm it is gone:

```powershell
Get-CimInstance Win32_Process -Filter "Name = 'SpeakLinkHQRuntime.exe'"
```

### 2. Verify the package you are rolling back TO

```powershell
.\Test-SpeakLinkHQPackage.ps1 -PackagePath "artifacts\<previous-package>"
```

Expect `SPEAKLINK_HQ_PACKAGE_VERIFIED`. A package that does not verify is not a
rollback target — it is a second problem.

### 3. Re-install it

```powershell
.\Install-SpeakLinkHQAutoStart.ps1 -PackagePath "artifacts\<previous-package>"
```
...
| --- | --- |
| `--target persistent` | `%LOCALAPPDATA%\SpeakLink\persistent-lan-server\backups\speaklink-before-password-change-<timestamp>.db` |
| `--target protected` | `%USERPROFILE%\speaklink-database-backups\speaklink_live-before-password-change-<timestamp>.db` |
...
Do not run `Initialize-SpeakLinkPersistentLanServer.ps1`. It creates a server;
running it against a damaged one is how an empty database that starts cleanly
replaces a real one.

```powershell
.\Test-SpeakLinkPersistentLanServer.ps1      # read-only diagnosis
.\Repair-SpeakLinkPersistentLanServer.ps1 -DryRun
```
