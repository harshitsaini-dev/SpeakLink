...
selected, or the shop could hear it, this computer cannot know. Only LinkGuard
...
```
%LOCALAPPDATA%\SpeakLink\receiver\device-credential.bin
```
...
token from `SPEAKLINK_RECEIVER_TOKEN`. It is never the default, prints a warning
