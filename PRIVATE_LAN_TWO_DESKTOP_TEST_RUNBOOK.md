...
```powershell
.\scripts\Install-EchoCastLanPilotFirewall.ps1
```
...
```powershell
.\scripts\Test-EchoCastLanPilotFirewall.ps1
```
...
```powershell
.\scripts\Test-EchoCastLanPilot.ps1
```
...
Expect `ECHOCAST_RECEIVER_FRESH_BUILD_VERIFIED`.
...
Expect `ECHOCAST_RECEIVER_PACKAGE_VERIFIED`.
...
Expect `ECHOCAST_STORE_PILOT_KIT_VERIFIED`.
...
`Install-EchoCastReceiverLanPilot.ps1` anywhere on the machine. That was a real
...
`C:\EchoCast\Kit`. Copy the folder, not just the `.exe`; the executable alone
will not run.

**B4.** On the Store desktop, confirm the copy is complete.

```powershell
Get-ChildItem C:\EchoCast\Receiver | Measure-Object
```

Expect 39 items, including `EchoCastReceiver.exe` and `ffmpeg.exe`.
...
```powershell
cd C:\EchoCast\Receiver
.\EchoCastReceiver.exe enrol `
...
EchoGuard hardware produces `SPEAKER_VERIFIED`.
...
```
Another EchoCast Receiver is already running for this Device on this computer.
```
...
```powershell
.\scripts\Install-EchoCastReceiverLanPilot.ps1 -PackagePath C:\EchoCast\Receiver
```
...
```powershell
Select-String -Path "$env:LOCALAPPDATA\EchoCast-AI\receiver\logs\receiver.log" `
              -Pattern 'echocast_rcv_v1\.', 'Bearer ', 'ECHO-'
```
...
```powershell
.\scripts\Stop-EchoCastLanPilot.ps1
```

**G4.** Remove the firewall rules (elevated).

```powershell
.\scripts\Uninstall-EchoCastLanPilotFirewall.ps1
```
...
multi-hour running, or any loudspeaker, amplifier, EchoGuard or
