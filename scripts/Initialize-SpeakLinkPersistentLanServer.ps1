...
.DESCRIPTION
    ...
    Start-SpeakLinkLanPilot.ps1 builds its data root from the clock:
    ...
    backend/speaklink_live.db, not in the pilot the backend was running).
...
Write-Output '=== SpeakLink persistent LAN server - initialization ==='
if (-not $Apply) {
...
if ($source.throwaway) {
    throw ("$SourceDatabase belongs to a throwaway pilot, which is rebuilt from " +
           ...
           'source database - normally backend\speaklink_live.db.')
}
...
Write-Output ''
Write-Output 'SPEAKLINK_PERSISTENT_SERVER_INITIALIZED'
Write-Output 'The source database was not deleted, moved or edited.'
Write-Output "Start it with .\scripts\Start-SpeakLinkPersistentLanServer.ps1"
