<#
.SYNOPSIS
    Verify the installed EchoCast HQ auto-start. Read-only.
...
Write-Output '=== EchoCast HQ auto-start verification ==='
...
Check 'EchoCastHQRuntime.exe is installed' { Test-Path $installedExe }
...
if ($task) {
    ...
    Check 'it runs EchoCastHQRuntime.exe directly' {
        ($task.Actions | ForEach-Object { $_.Execute }) -match 'EchoCastHQRuntime\.exe'
    }
