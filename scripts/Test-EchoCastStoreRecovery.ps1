...
Write-Output '=== EchoCast Store recovery diagnostics (read-only) ==='
Write-Output ''
...
if (-not $task) {
    ...
    $windowed = $action.Execute -match 'EchoCastReceiverBackground\.exe'
...
Line 'background exe' $(if (Test-Path (Join-Path $InstallRoot 'EchoCastReceiverBackground.exe')) { 'present' } else { 'MISSING' })
Line 'operator exe' $(if (Test-Path (Join-Path $InstallRoot 'EchoCastReceiver.exe')) { 'present' } else { 'MISSING' })
...
$background = @(Get-CimInstance Win32_Process -Filter "Name = 'EchoCastReceiverBackground.exe'" -ErrorAction SilentlyContinue)
$console = @(Get-CimInstance Win32_Process -Filter "Name = 'EchoCastReceiver.exe'" -ErrorAction SilentlyContinue |
...
if (-not (Test-Path $logDirectory)) {
    ...
    if ($newest) {
        ...
        Get-Content $newest.FullName -Tail 12 | ForEach-Object {
            $safe = $_ -replace 'echocast_rcv_v1\.[A-Za-z0-9._\-]+', '<REDACTED credential>' `
...
Write-Output ''
Write-Output 'ECHOCAST_STORE_RECOVERY_DIAGNOSTICS_COMPLETE'
Write-Output ''
