<#
.SYNOPSIS
    Remove the EchoCast HQ auto-start and its application files. Keep the data.
...
.DESCRIPTION
    ...
    That the task runs an EchoCast HQ runtime, and that a process to be stopped
...
Write-Output '=== removing EchoCast HQ auto-start ==='
...
if (-not $task) {
    ...
    if ($ran -notmatch 'EchoCastHQRuntime\.exe') {
        throw ("The task '$TaskName' runs '$ran', which is not an EchoCast HQ " +
...
$others = @(Get-CimInstance Win32_Process -Filter "Name = 'EchoCastHQRuntime.exe'" |
...
if ($DryRun -or -not $PSCmdlet.ShouldProcess($TaskName, 'Remove EchoCast HQ auto-start')) {
    Write-Output ''
...
$statusFile = Join-Path $env:LOCALAPPDATA 'EchoCast-AI\hq-runtime-status.json'
if (Test-Path $statusFile) { Remove-Item $statusFile -Force }

Write-Output ''
Write-Output 'ECHOCAST_HQ_AUTO_START_REMOVED'
Write-Output ''
