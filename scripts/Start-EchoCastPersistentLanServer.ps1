...
.DESCRIPTION
    The difference from Start-EchoCastLanPilot.ps1 is the whole point:

      pilot       ...\lan-pilot\<timestamp>\lan-pilot.db   + a new random admin
      persistent  ...\persistent-lan-server\data\echocast.db  + the accounts already in it
...
Write-Output '=== EchoCast PERSISTENT LAN SERVER ==='
...
if (-not (Test-Path $serverProfile.database)) {
    throw ("There is no persistent database at $($serverProfile.database). " +
           ...
           '.\scripts\Initialize-EchoCastPersistentLanServer.ps1 -SourceDatabase ' +
           '"backend\echocast_live.db" first.')
}
...
if ($busy.Count -gt 0) {
    throw ("Port $BackendPort is already in use, so a server is already running - " +
           ...
           '.\scripts\Stop-EchoCastPersistentLanServer.ps1 or ' +
...
$environment = @{
    ECHOCAST_DB_PATH       = $serverProfile.database
    ECHOCAST_KEY_CONTAINER = $serverProfile.keys
    ECHOCAST_KEY_PROTECTOR = 'dpapi'
    ECHOCAST_SERVER_MODE   = $serverProfile.mode
...
Write-Output ''
Write-Output 'ECHOCAST_PERSISTENT_SERVER_STARTED'
