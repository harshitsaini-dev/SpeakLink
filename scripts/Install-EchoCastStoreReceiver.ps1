<#
.SYNOPSIS
    Install the EchoCast Receiver as a background Store runtime.
...
.DESCRIPTION
    ...
    The scheduled task runs EchoCastReceiverBackground.exe, which is built
    ...
    application putting a black window on the counter. EchoCastReceiver.exe -
...
Write-Output '=== installing the EchoCast Store Receiver ==='
...
$consoleExe = Join-Path $PackagePath 'EchoCastReceiver.exe'
$backgroundExe = Join-Path $PackagePath 'EchoCastReceiverBackground.exe'
...
Write-Output "  runs        : EchoCastReceiverBackground.exe (windowed - no console window)"
...
if ($AudioSink -eq 'windows' -and -not $DryRun) {
    ...
    try {
        $probe = & $consoleExe run --backend-url $BackendUrl --audio-sink windows `
                    ...
                    --credential-path (Join-Path $env:TEMP 'echocast-install-probe.bin') `
                    --config-path (Join-Path $env:TEMP 'echocast-install-probe.json') `
...
foreach ($pattern in @('echocast_rcv_v1', 'ECHO-[A-Z0-9]{4}-', '(?i)"password"', '(?i)"credential"')) {
...
if (Test-Path $credentialPath) {
    ...
    Write-Output "    & `"$InstallRoot\EchoCastReceiver.exe`" enrol --backend-url $BackendUrl ``"
...
# The earlier pilot task runs EchoCastReceiver.exe - the CONSOLE build - so a
...
foreach ($obsolete in @('EchoCast Receiver LAN Pilot (disposable)',
                        'EchoCast Store Probe',
                        'EchoCast Restart Probe')) {
    ...
    foreach ($process in @(Get-CimInstance Win32_Process -Filter "Name = 'EchoCastReceiver.exe'" |
...
# stays a bare "run".
$arguments = 'run'
if ($StateRoot -ne (Join-Path $env:LOCALAPPDATA 'EchoCast-AI\receiver')) {
    $arguments += " --config-path `"$configPath`" --credential-path `"$credentialPath`""
}

$action = New-ScheduledTaskAction -Execute (Join-Path $InstallRoot 'EchoCastReceiverBackground.exe') `
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $triggers `
    ...
    -Description 'EchoCast Live Store Receiver. Starts at logon, runs with no window.' | Out-Null

$xml = Export-ScheduledTask -TaskName $TaskName
foreach ($pattern in @('echocast_rcv_v1', 'ECHO(-[A-Z0-9]{4}){2,}', '(?i)password', '(?i)bearer')) {
...
if (Test-Path $credentialPath) {
    ...
    $live = @(Get-CimInstance Win32_Process -Filter "Name = 'EchoCastReceiverBackground.exe'" |
              Where-Object { $_.ExecutablePath -and $_.ExecutablePath.StartsWith($InstallRoot, 'OrdinalIgnoreCase') })
    if ($live.Count -gt 0) {
...
Write-Output ''
Write-Output 'ECHOCAST_STORE_RECEIVER_INSTALLED'
Write-Output "Verify   : .\Test-EchoCastStoreReceiver.ps1 -TaskName `"$TaskName`""
Write-Output "Diagnose : & `"$InstallRoot\EchoCastReceiver.exe`" diagnose"
