<#
.SYNOPSIS
    Verify a built SpeakLinkReceiver package the way a Store desktop would meet it.
...
if (-not $PackagePath) { $PackagePath = Join-Path $repositoryRoot "artifacts\SpeakLinkReceiver-$Version" }
...
Write-Output '=== SpeakLinkReceiver package verification ==='
Write-Output "  package: $PackagePath"

# ---------------------------------------------------------------------------
# Contents
# ---------------------------------------------------------------------------
Check 'SpeakLinkReceiver.exe present'   { Test-Path (Join-Path $PackagePath 'SpeakLinkReceiver.exe') }
...
Check 'no credential, key or password in any package file' {
    $patterns = @('speaklink_rcv_v', 'eyJ[A-Za-z0-9_-]{10}', '\$2[aby]\$[0-9]{2}\$',
...
# ---------------------------------------------------------------------------
$sandbox = Join-Path $env:TEMP ("SpeakLink Package Check " + (Get-Date -Format 'yyyyMMdd-HHmmss'))
New-Item -ItemType Directory -Force -Path $sandbox | Out-Null
Copy-Item $PackagePath (Join-Path $sandbox 'SpeakLinkReceiver') -Recurse -Force
$exe = Join-Path $sandbox 'SpeakLinkReceiver\SpeakLinkReceiver.exe'
...
try {
    ...
    Check '--version answers' {
        ...
        $LASTEXITCODE -eq 0 -and $out -match 'SpeakLinkReceiver'
    }
...
if ($failed.Count -eq 0) {
    Write-Output 'Result: SPEAKLINK_RECEIVER_PACKAGE_VERIFIED'
