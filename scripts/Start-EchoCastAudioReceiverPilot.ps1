...
.NOTES
    The Store credential must be supplied through ECHOCAST_RECEIVER_TOKEN in
...
Write-Output '=== EchoCast local audio Receiver pilot (NULL sink) ==='
...
if ([string]::IsNullOrWhiteSpace($env:ECHOCAST_RECEIVER_TOKEN)) {
    throw 'ECHOCAST_RECEIVER_TOKEN is not set for this PowerShell session. Copy the Store credential from Store Management and set it before starting. It is never stored by this script.'
}
...
Write-Output ''
Write-Output 'Stop it with: .\scripts\Stop-EchoCastAudioReceiverPilot.ps1'
