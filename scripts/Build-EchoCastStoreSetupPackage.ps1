<#
.SYNOPSIS
    Package EchoCastStoreSetup.exe as a versioned, hashed artifact.

.DESCRIPTION
    EchoCastStoreSetup.exe was being built into dist\ and shipped in nothing.
    ...
    Test-EchoCastStorePilotKit.ps1; whether the wizard belongs inside it is an
...
if (-not $DistPath) { $DistPath = Join-Path $repositoryRoot 'dist\EchoCastStoreSetup' }
...
Write-Output '=== building an EchoCast Store Setup package ==='
...
$packageName = "EchoCastStoreSetup-$Version-$shortCommit-$timestamp"
...
$setupExe = Join-Path $DistPath 'EchoCastStoreSetup.exe'
if (-not (Test-Path $setupExe)) {
    throw "There is no EchoCastStoreSetup.exe in $DistPath."
}
...
if ($newestSource -and $builtAt -lt $newestSource) {
    throw ("EchoCastStoreSetup.exe was built at $builtAt but a source input " +
...
EchoCastStoreSetup.exe sets this computer up as an EchoCast Receiver. It asks
...
2. RUN IT
   ...
   Double-click EchoCastStoreSetup.exe.
...
PRIVATE NETWORK ONLY
   ...
   EchoCast runs on the shop's own network. This package is not for use over
   the public internet.
"@
...
Write-Output ''
Write-Output 'ECHOCAST_STORE_SETUP_PACKAGE_BUILT'
