<#
.SYNOPSIS
    Create the dedicated SpeakLinkService account and lock down the Receiver HMAC
...
.DESCRIPTION
    Implements the recorded architecture decision:
      ...
      - one Windows server
      - one dedicated local account, SpeakLinkService
      - DPAPI CURRENT_USER scope under that account
      - key container at C:\ProgramData\SpeakLink\keys\receiver-hmac-keys.bin
      - ACL: SpeakLinkService read/write, Administrators recovery, nobody else
...
param(
    ...
    [string] $KeyDirectory = 'C:\ProgramData\SpeakLink\keys',
    [switch] $WhatIfOnly
)
...
Write-Output '=== SpeakLink service identity and key directory ==='
Assert-Elevated
...
Write-Output ''
Write-Output 'Next, as SpeakLinkService:'
Write-Output '  .\scripts\Test-SpeakLinkKeyCustody.ps1'
