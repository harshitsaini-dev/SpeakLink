<#
.SYNOPSIS
    Prepare the dedicated SpeakLinkService account and the SpeakLink directories
    on the backend host. Run once, elevated.

.DESCRIPTION
    Implements the recorded architecture decision: one Windows server, one
    dedicated local account SpeakLinkService, DPAPI CURRENT_USER scope under that
    account, and the SpeakLink tree under C:\ProgramData\SpeakLink.
...
.EXAMPLE
    .\Install-SpeakLinkServiceIdentity.ps1 -WhatIfOnly

.EXAMPLE
    .\Install-SpeakLinkServiceIdentity.ps1
#>
[CmdletBinding()]
param(
    [string] $AccountName = 'SpeakLinkService',
    [string] $Root = 'C:\ProgramData\SpeakLink',
...
function Test-SpeakLinkDirectoryAcl {
...
Write-Output '=== SpeakLink service identity and directories ==='
Write-Output ''
...
if ($existing) {
    ...
    New-LocalUser -Name $AccountName -Password $secret `
        -Description 'SpeakLink backend service identity. No interactive logon.' `
...
foreach ($entry in $directories) {
    ...
    if (Test-SpeakLinkDirectoryAcl -Path $path -Role $entry.Role) {
...
if ($allVerified) {
    ...
    Write-Output 'Next, as the service account: .\scripts\Test-SpeakLinkKeyCustody.ps1'
    exit 0
}
