<#
.SYNOPSIS
    Start EchoCast in LOCAL PILOT MODE against the isolated pilot database.
...
.DESCRIPTION
    ...
    The protected application database backend\echocast_live.db is never used.
...
$pilotData = Join-Path $PilotRoot 'data\echocast_local_pilot.db'
...
$env:ECHOCAST_DB_PATH = $pilotData
...
Write-Output ''
Write-Output 'The protected database backend\echocast_live.db is NOT in use.'
...
Write-Output ''
Write-Output 'Stop it with: .\scripts\Stop-EchoCastLocalPilot.ps1'
