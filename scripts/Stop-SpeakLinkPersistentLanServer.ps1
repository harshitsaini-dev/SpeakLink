...
Write-Output '=== stopping the SpeakLink persistent LAN server ==='

if (-not (Test-Path $serverProfile.lock)) {
    Write-Output '  no lock file - this server does not appear to be running'
    Write-Output ''
    Write-Output 'SPEAKLINK_PERSISTENT_SERVER_NOT_RUNNING'
    exit 0
}
...
Write-Output ''
Write-Output 'SPEAKLINK_PERSISTENT_SERVER_STOPPED'
