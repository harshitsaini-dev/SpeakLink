<#
.SYNOPSIS
    Verify the disposable EchoCast Receiver logon task, including the claims it
...
Write-Output '=== EchoCast Receiver logon task verification ==='

$task = Get-ScheduledTask -TaskName $TaskName -ErrorAction SilentlyContinue
if (-not $task) {
    throw "No scheduled task named '$TaskName'. Install one with .\scripts\Install-EchoCastReceiverLanPilot.ps1."
}
...
# ---------------------------------------------------------------------------
Check 'no Device credential in the task definition' { $xml -notmatch 'echocast_rcv_v1\.' }
...
if ($IncludeLiveInstanceCheck) {
    Check 'a duplicate launch of the packaged EXE exits 4' {
        ...
        $previous = $ErrorActionPreference; $ErrorActionPreference = 'Continue'
        try {
            $sandbox = Join-Path $env:TEMP ("EchoCast Instance Check " + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
Write-Output ''
Write-Output 'ECHOCAST_RECEIVER_TASK_SCHEDULER_VERIFIED'
Write-Output ''
