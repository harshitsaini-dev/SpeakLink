...
Write-Output '=== repairing the EchoCast persistent LAN server ==='
...
if (-not (Test-Path $serverProfile.root)) {
    throw ("There is no persistent server at $($serverProfile.root). Repair cannot " +
           'create one - run Initialize-EchoCastPersistentLanServer.ps1 instead.')
}
...
Write-Output ''
Write-Output 'ECHOCAST_PERSISTENT_SERVER_REPAIRED'
Write-Output 'The database, keys, users, Stores, Devices, history and backups were'
Write-Output 'not touched. Verify with .\scripts\Test-EchoCastPersistentLanServer.ps1'
