...
if ($running.Count -eq 0) {
    ...
    # they are stopped in is not arbitrary. EchoCastReceiverBackground is the
    ...
    $ordered = @($running | Sort-Object -Property @{
        Expression = { if ($_.Name -eq 'EchoCastReceiverBackground.exe') { 0 } else { 1 } }
    })
    ...
    # a still-locked EchoCastReceiver.exe and the whole uninstall fails on a
    # race rather than on anything real.
    $deadline = (Get-Date).AddSeconds(20)
    while ((Get-Date) -lt $deadline) {
        $stillRunning = @(Get-CimInstance Win32_Process -Filter "Name = 'EchoCastReceiverBackground.exe' OR Name = 'EchoCastReceiver.exe'" |
                          Where-Object { $_.ExecutablePath -and $_.ExecutablePath.StartsWith($InstallRoot, 'OrdinalIgnoreCase') })
