<#
.SYNOPSIS
    Query, start or stop the EchoCast Store Receiver task. Nothing else.

.DESCRIPTION
    WHY THIS EXISTS AS ITS OWN SCRIPT

    EchoCastStoreSetup.exe's Rerun screen needs to start, stop and check the
    ...
    names EchoCastReceiverBackground.exe - the same check the auto-start and
...
function Test-IsOurTask {
    ...
    return $executed -match 'EchoCastReceiverBackground\.exe'
}
...
if (-not (Test-IsOurTask $task)) {
    ...
    throw ("The task '$TaskName' runs '$ran', which is not " +
           "EchoCastReceiverBackground.exe - refusing: this task is not ours.")
}
