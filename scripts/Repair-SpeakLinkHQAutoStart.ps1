<#
.SYNOPSIS
    Put a damaged SpeakLink HQ installation back, without touching HQ's data.
...
Write-Output '=== repairing the SpeakLink HQ installation ==='
...
if ((Get-PeSubsystem (Join-Path $PackagePath 'SpeakLinkHQRuntime.exe')) -ne 2) {
...
$persistentReport = [ordered]@{
    'database' = (Join-Path $PersistentRoot 'data\speaklink.db')
...
if ($missingData -contains 'database' -or $missingData -contains 'keys') {
    throw ("HQ's own data is missing ($($missingData -join ', ')). This is not an " +
           ...
           'Test-SpeakLinkPersistentLanServer.ps1 to see what survived. Nothing was changed.')
}
...
if (-not $task) {
    ...
    if ($ran -notmatch 'SpeakLinkHQRuntime\.exe') {
        throw ("A task named '$TaskName' exists and runs '$ran', which is not an " +
               'SpeakLink HQ runtime. Refusing to modify a task that is not ours.')
    }
    $wrong = @()
...
if ($broken.Count -eq 0 -and $taskVerdict -eq 'correct') {
    ...
    Write-Output 'SPEAKLINK_HQ_REPAIR_NOT_NEEDED'
    exit 0
}

if ($DryRun -or -not $PSCmdlet.ShouldProcess($TaskName, 'Repair the SpeakLink HQ installation')) {
    Write-Output ''
...
# ---------------------------------------------------------------------------
foreach ($process in @(Get-CimInstance Win32_Process -Filter "Name = 'SpeakLinkHQRuntime.exe'" |
...
Write-Output ''
Write-Output 'SPEAKLINK_HQ_REPAIRED'
