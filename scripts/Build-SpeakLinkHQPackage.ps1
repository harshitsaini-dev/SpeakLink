<#
.SYNOPSIS
    Build a versioned, hashed SpeakLink HQ package that another machine can install.

.DESCRIPTION
    WHAT A PACKAGE IS FOR

    Everything an HQ computer needs to run SpeakLink, and nothing that belongs to
    ...
    Test-SpeakLinkHQPackage.ps1 then goes looking for what should not be there -
...
$packageName = "SpeakLinkHQ-$Version-$shortCommit-$timestamp"
...
$runtimeExe = Join-Path $RuntimePath 'SpeakLinkHQRuntime.exe'
if (-not (Test-Path $runtimeExe)) {
    throw "There is no SpeakLinkHQRuntime.exe in $RuntimePath."
}
...
$taskScripts = @('Install-SpeakLinkHQAutoStart.ps1', 'Test-SpeakLinkHQAutoStart.ps1',
                 'Repair-SpeakLinkHQAutoStart.ps1', 'Uninstall-SpeakLinkHQAutoStart.ps1')
...
1. VERIFY WHAT YOU RECEIVED
    ...
    .\Test-SpeakLinkHQPackage.ps1 -PackagePath .

   Expect: SPEAKLINK_HQ_PACKAGE_VERIFIED

2. INSTALL THE AUTO-START (registers, does not start)

    .\Install-SpeakLinkHQAutoStart.ps1 -PackagePath . -DryRun
    .\Install-SpeakLinkHQAutoStart.ps1 -PackagePath .

   Expect: SPEAKLINK_HQ_AUTO_START_INSTALLED
...
IF SOMETHING IS WRONG
    ...
    .\Test-SpeakLinkHQAutoStart.ps1          what is installed and running
    .\Repair-SpeakLinkHQAutoStart.ps1 -PackagePath . -DryRun    what it would fix
    .\Uninstall-SpeakLinkHQAutoStart.ps1     remove the app, KEEP the data
...
$manifest = [ordered]@{
    ...
    contains = @('SpeakLinkHQRuntime.exe', 'frontend/ (production build)',
...
Write-Output ''
Write-Output 'SPEAKLINK_HQ_PACKAGE_BUILT'
Write-Output "Package: $outputPath"
Write-Output "Verify : .\Test-SpeakLinkHQPackage.ps1 -PackagePath `"$outputPath`""
