...
Write-Output '=== EchoCast persistent LAN server verification ==='
...
Check 'no secret is written into the lock file' {
    ...
    $text -notmatch '(?i)(password|secret|token|bearer|echocast_rcv_v1)'
}
...
Write-Output ''
Write-Output 'ECHOCAST_PERSISTENT_SERVER_VERIFIED'
Write-Output ''
