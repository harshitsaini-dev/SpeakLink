...
Write-Output '=== EchoCast Store Receiver verification ==='
...
Check 'the background executable is installed' {
    Test-Path (Join-Path $InstallRoot 'EchoCastReceiverBackground.exe')
}
Check 'the operator executable is installed' {
    Test-Path (Join-Path $InstallRoot 'EchoCastReceiver.exe')
}
...
Check 'the background executable creates NO console window' {
    (Get-PeSubsystem (Join-Path $InstallRoot 'EchoCastReceiverBackground.exe')) -eq 2
}
Check 'the operator executable still has a console' {
    (Get-PeSubsystem (Join-Path $InstallRoot 'EchoCastReceiver.exe')) -eq 3
}
...
Check 'the settings contain no secret' {
    ...
    $text -notmatch 'echocast_rcv_v1' -and $text -notmatch 'ECHO(-[A-Z0-9]{4}){2,}' -and
...
if ($task) {
    ...
    Check 'the task carries no credential, code or password' {
        ...
        $xml -notmatch 'echocast_rcv_v1' -and $xml -notmatch 'ECHO(-[A-Z0-9]{4}){2,}' -and
...
Check 'no secret in any log' {
    ...
    @(Get-ChildItem $logDirectory -File |
      Select-String -Pattern 'echocast_rcv_v1', 'ECHO(-[A-Z0-9]{4}){2,}', 'Bearer ' -ErrorAction SilentlyContinue).Count -eq 0
}
...
Write-Output ''
Write-Output 'ECHOCAST_STORE_RECEIVER_VERIFIED'
Write-Output ''
