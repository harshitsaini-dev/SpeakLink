<#
.SYNOPSIS
    Register the SpeakLink HQ Runtime to start when the HQ user signs in.
...
.DESCRIPTION
    ...
    SpeakLinkHQRuntime.exe gives up after a bounded number of failed starts and
...
Write-Output '=== installing SpeakLink HQ auto-start ==='
...
$runtimeExe = Join-Path $PackagePath 'SpeakLinkHQRuntime.exe'
...
if ($subsystem -ne 2) {
    throw ("SpeakLinkHQRuntime.exe in that package is a console application " +
...
foreach ($required in @($persistentDatabase, $persistentKeys, $persistentSigning)) {
    if (-not (Test-Path $required)) {
        throw ("The persistent HQ profile is not initialized: $required is " +
               "missing. Run Initialize-SpeakLinkPersistentLanServer.ps1 first. " +
...
if ($DryRun -or -not $PSCmdlet.ShouldProcess($TaskName, 'Install SpeakLink HQ auto-start')) {
    Write-Output ''
...
# ---------------------------------------------------------------------------
$running = @(Get-CimInstance Win32_Process -Filter "Name = 'SpeakLinkHQRuntime.exe'" |
...
if ($existing) {
    ...
    Write-Output '  replacing the earlier SpeakLink task of the same name'
...
Register-ScheduledTask -TaskName $TaskName -Action $action `
    ...
    -Description 'SpeakLink HQ Runtime. Starts at sign-in, runs with no window.' | Out-Null

# Checking the variables is not checking the task. Read back what Windows
# actually stored.
$xml = Export-ScheduledTask -TaskName $TaskName
foreach ($pattern in @('speaklink_rcv_v1', 'ECHO(-[A-Z0-9]{4}){2,}',
...
if ($StartNow) {
    ...
    if ($live.Count -gt 0) {
        ...
        Write-Output "    $env:LOCALAPPDATA\SpeakLink\hq-runtime-status.json"
...
Write-Output ''
Write-Output 'SPEAKLINK_HQ_AUTO_START_INSTALLED'
Write-Output "Verify : .\Test-SpeakLinkHQAutoStart.ps1 -TaskName `"$TaskName`" -InstallRoot `"$InstallRoot`""
Write-Output "Status : $env:LOCALAPPDATA\SpeakLink\hq-runtime-status.json"
