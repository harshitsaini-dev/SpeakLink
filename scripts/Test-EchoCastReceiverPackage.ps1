...
try {
    ...
    Check 'the background executable is present' {
        Test-Path (Join-Path $sandbox 'EchoCastReceiver\EchoCastReceiverBackground.exe')
    }
    Check 'the background executable creates NO console window' {
        (Get-PeSubsystem (Join-Path $sandbox 'EchoCastReceiver\EchoCastReceiverBackground.exe')) -eq 2
    }
    Check 'the operator executable still HAS a console' {
        ...
        (Get-PeSubsystem (Join-Path $sandbox 'EchoCastReceiver\EchoCastReceiver.exe')) -eq 3
    }
