...
.DESCRIPTION
    Requires both ECHOCAST_AUDIO_SINK_MODE=windows and an exact
    ECHOCAST_AUDIO_OUTPUT_DEVICE selector. It prints the selected device before
    ...
    unavailable until EchoGuard exists.

.NOTES
    The Store credential must be in ECHOCAST_RECEIVER_TOKEN in this session. It
...
Write-Output '=== EchoCast audio Receiver pilot (WINDOWS OUTPUT DEVICE) ==='
...
if ($env:ECHOCAST_AUDIO_SINK_MODE -ne 'windows') {
    throw "ECHOCAST_AUDIO_SINK_MODE must be 'windows' for hardware mode. It is currently '$($env:ECHOCAST_AUDIO_SINK_MODE)'. Use Start-EchoCastAudioReceiverPilot.ps1 for the safe null sink."
}
...
if ([string]::IsNullOrWhiteSpace($env:ECHOCAST_RECEIVER_TOKEN)) {
    throw 'ECHOCAST_RECEIVER_TOKEN is not set for this PowerShell session. Copy the Store credential from Store Management. It is never stored by this script.'
}
...
Write-Output ''
Write-Output "Selected output device: $($env:ECHOCAST_AUDIO_OUTPUT_DEVICE)"
Write-Output 'Confirm this is the wired adapter feeding the amplifier AUX input.'
Write-Output 'Run .\scripts\List-EchoCastAudioDevices.ps1 if you are unsure.'
Write-Output ''
...
$receiver = Start-Process -FilePath $venvPython `
    ...
    -PassThru
Write-EchoCastPidFile -PidFile $pidFile -ProcessId $receiver.Id
...
Write-Output ''
Write-Output 'Stop it with: .\scripts\Stop-EchoCastWindowsAudioReceiverPilot.ps1'
