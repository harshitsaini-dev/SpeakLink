...
if ($task) {
    ...
    Check 'it does NOT run the console executable' {
        ...
        ($task.Actions | ForEach-Object { $_.Execute }) -notmatch 'SpeakLinkReceiver\.exe$'
    }
...
Check 'no obsolete SpeakLink task survives' {
    # A leftover pilot task runs the console build and shows a window at logon.
    @(Get-ScheduledTask | Where-Object {
        $_.TaskName -like '*SpeakLink*' -and $_.TaskName -ne $TaskName
    }).Count -eq 0
}
