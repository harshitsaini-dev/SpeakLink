...
$RULES = @(
    @{ Name = "$RULE_PREFIX - HQ dashboard (TCP 3000)"; Port = 3000
       Description = 'EchoCast private LAN pilot. HQ dashboard. Private profile, local subnet only. Remove with Uninstall-EchoCastLanPilotFirewall.ps1.' }
    @{ Name = "$RULE_PREFIX - API (TCP 8000)";          Port = 8000
       Description = 'EchoCast private LAN pilot. FastAPI backend. Private profile, local subnet only. Remove with Uninstall-EchoCastLanPilotFirewall.ps1.' }
)
...
Write-Output '=== EchoCast LAN pilot firewall rules ==='
Write-Output ''
...
Write-Output ''
Write-Output 'Done. Verify with .\scripts\Test-EchoCastLanPilotFirewall.ps1'
Write-Output 'Remove with  .\scripts\Uninstall-EchoCastLanPilotFirewall.ps1'
