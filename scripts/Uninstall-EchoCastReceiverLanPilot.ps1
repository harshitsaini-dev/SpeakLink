<#
.SYNOPSIS
    Remove the disposable EchoCast Receiver logon task, and only that task.
...
.DESCRIPTION
    ...
    action actually points at an EchoCastReceiver executable before removing it.
    ...
    `EchoCastReceiver.exe remove-local-credential` for that, which asks first.
#>
...
Write-Output '=== removing the EchoCast Receiver logon task ==='

$task = Get-ScheduledTask -TaskName $TaskName -ErrorAction SilentlyContinue
if (-not $task) {
    Write-Output "  no scheduled task named '$TaskName' is registered"
    Write-Output ''
    Write-Output 'ECHOCAST_RECEIVER_TASK_ABSENT'
    exit 0
}

$execute = ($task.Actions | ForEach-Object { $_.Execute }) -join ' '
if ($execute -notmatch 'EchoCastReceiver') {
    throw ("The task '$TaskName' runs '$execute', which is not an EchoCastReceiver " +
...
if ($StopRunning) {
    ...
    $running = @(Get-CimInstance Win32_Process -Filter "Name = 'EchoCastReceiver.exe'" |
...
Write-Output ''
Write-Output 'ECHOCAST_RECEIVER_TASK_REMOVED'
Write-Output 'The DPAPI Device credential was NOT removed. That is a separate,'
Write-Output 'confirmed decision: EchoCastReceiver.exe remove-local-credential'
