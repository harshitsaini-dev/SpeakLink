...
.NOTES
    ...
    its return value, so a Write-Output inside Stop-EchoCastProcessTree turns
    ...
    Test-EchoCastPortReleased report success for a port that was still bound.
    backend/tests/test_pilot_scripts.py pins it.
#>

function Read-EchoCastPidFile {
    <#
...
function Write-EchoCastPidFile {
...
function Get-EchoCastProcessTable {
...
function Stop-EchoCastProcessTree {
    <#
    ...
    $record = Read-EchoCastPidFile -PidFile $PidFile
    if ($null -eq $record) {
    ...
    $table = Get-EchoCastProcessTable
...
function Test-EchoCastPortReleased {
