...
Write-Output '=== SpeakLink HQ package verification ==='
...
foreach ($expected in @('SpeakLinkHQRuntime.exe', 'manifest.json', 'SHA256SUMS.txt',
                        'QUICK-START.txt', 'frontend\index.html', 'backend\server.py',
                        'Install-SpeakLinkHQAutoStart.ps1', 'Test-SpeakLinkHQAutoStart.ps1',
...
$allowedExecutables = @('SpeakLinkHQRuntime.exe')
...
Check 'the install script runs SpeakLinkHQRuntime.exe directly' {
    ...
    $body = Get-Content (Join-Path $PackagePath 'Install-SpeakLinkHQAutoStart.ps1') -Raw
    ...
    ($body -match "\`$installedExe = Join-Path \`$InstallRoot 'SpeakLinkHQRuntime\.exe'") -and
...
Check 'no credential, code or token in any packaged text' {
    @(Get-ChildItem $PackagePath -Recurse -File -Include '*.ps1', '*.txt', '*.json', '*.py', '*.md' |
      Select-String -Pattern 'speaklink_rcv_v1\.', 'ECHO(-[A-Z0-9]{4}){2,}', 'eyJ[A-Za-z0-9_-]{20,}' `
        -ErrorAction SilentlyContinue).Count -eq 0
}
...
Check 'the package name matches the version and commit it records' {
    ...
    $leaf.StartsWith("SpeakLinkHQ-$($manifest.version)-$($manifest.source_commit_short)")
}
...
Write-Output ''
Write-Output 'SPEAKLINK_HQ_PACKAGE_VERIFIED'
Write-Output ''
...
Write-Output 'Test-SpeakLinkHQAutoStart.ps1 there.'
exit 0
