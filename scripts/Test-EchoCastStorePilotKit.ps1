...
Write-Output '=== EchoCast Store pilot kit verification ==='
...
foreach ($required in @('EchoCastReceiver.exe', 'ffmpeg.exe', 'manifest.json', 'SHA256SUMS.txt', '_internal', 'licenses')) {
    Check "Receiver\$required present" { Test-Path (Join-Path $receiver $required) }
}

# The gap this kit exists to close.
foreach ($name in @('Install-EchoCastReceiverLanPilot.ps1',
...
Check 'the executable is newer than nothing it should predate' {
    $exeTime = (Get-Item (Join-Path $receiver 'EchoCastReceiver.exe')).LastWriteTime
...
Check 'no credential, code, password or key anywhere in the kit' {
    $patterns = @('echocast_rcv_v1\.', 'ECHO(-[A-Z0-9]{4}){2,}', 'BEGIN [A-Z ]*PRIVATE KEY',
...
Check 'README-FIRST contains no credential or code' {
    $readme -notmatch 'echocast_rcv_v1\.' -and $readme -notmatch 'ECHO(-[A-Z0-9]{4}){2,}'
}
...
Check 'the packaged executable answers --version from the kit' {
    ...
    try { ((& (Join-Path $receiver 'EchoCastReceiver.exe') --version) -join ' ') -match '\d+\.\d+\.\d+' }
...
Write-Output ''
Write-Output 'ECHOCAST_STORE_PILOT_KIT_VERIFIED'
Write-Output ''
