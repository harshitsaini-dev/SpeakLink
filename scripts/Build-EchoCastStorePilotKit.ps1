...
Write-Output '=== building the EchoCast Store pilot kit ==='
...
if ((Split-Path -Leaf $PackagePath) -eq 'EchoCastReceiver-1.0.0') {
    throw 'That is the original stale package. Build a fresh one with .\scripts\Build-EchoCastReceiver.ps1.'
}

foreach ($required in @('EchoCastReceiver.exe', 'ffmpeg.exe', 'manifest.json', 'SHA256SUMS.txt', '_internal')) {
...
if (-not $OutputRoot) {
    $OutputRoot = Join-Path $repositoryRoot "artifacts\EchoCast-Store-Pilot-$shortCommit-$stamp"
}
...
STEP 3 - enrol this computer (you will be asked for the code; it is not shown
    ...
    .\Receiver\EchoCastReceiver.exe enrol ``
...
STEP 4 - check what was stored
    ...
    .\Receiver\EchoCastReceiver.exe status
...
STEP 5 - CHOOSE WHERE THE SOUND GOES. Do not skip this.
  ...
  List the sound outputs on THIS computer:
    ...
    .\Receiver\EchoCastReceiver.exe list-audio-devices
...
STEP 6 - run it once by hand, with sound, and watch it
    ...
    .\Receiver\EchoCastReceiver.exe run ``
        --backend-url http://192.168.4.134:8000 ``
        --allow-insecure-private-lan --expected-hq-host 192.168.4.134 ``
        --audio-sink windows ``
...
WHERE THINGS LIVE
  ...
  sealed credential  : %LOCALAPPDATA%\EchoCast-AI\receiver\device-credential.bin
  logs               : %LOCALAPPDATA%\EchoCast-AI\receiver\logs
...
[PSCustomObject]@{
    product = 'EchoCast Store Pilot Kit'
    ...
    installs_to = '%LOCALAPPDATA%\EchoCast-AI\receiver-app'
    credential_path = '%LOCALAPPDATA%\EchoCast-AI\receiver\device-credential.bin'
    log_directory = '%LOCALAPPDATA%\EchoCast-AI\receiver\logs'
...
Write-Output ''
Write-Output 'ECHOCAST_STORE_PILOT_KIT_BUILT'
Write-Output "Kit: $OutputRoot"
Write-Output "Verify with .\scripts\Test-EchoCastStorePilotKit.ps1 -KitPath `"$OutputRoot`""
