...
`SPEAKER_VERIFIED`: nothing measured the sound, and EchoGuard does not exist.
...
`backend/echocast_live.db` was never opened, copied or modified. Every run used
the isolated pilot database under
`%LOCALAPPDATA%\EchoCast-AI\local-pilot\`, which carries a `PILOT_ONLY` marker
...
```
python.exe: can't open file 'C:\Users\admin\Desktop\EchoCast-AI\HQ-Broadcast-Full'
```
...
`4aa9b2a`. `Stop-EchoCastLocalPilot.ps1` stopped only the PID recorded at
...
- Progress messages used `Write-Output` inside functions that return a bool.
  ...
  `Test-EchoCastPortReleased` reported success for a port that was still bound.
