...
versioned token format `echocast_rcv_v1.<uuid>.<secret>` with HMAC-SHA256
...
4. Agent stores the credential under %LOCALAPPDATA%\EchoCast-AI, outside Git
...
4. Windows Receiver agent enrolment: read a code from stdin, store the
   credential under `%LOCALAPPDATA%\EchoCast-AI`, reconnect without
