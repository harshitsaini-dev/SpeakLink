# EchoCast Local One-Store Pilot Test Runbook
...
`backend\echocast_live.db` is never used, opened, copied or modified.
...
4. **No EchoCast backend or frontend already running.** Check first:
   ```powershell
...
Local pilot database prepared (isolated, disposable).
  ...
  database_path: C:\Users\<you>\AppData\Local\EchoCast-AI\local-pilot\data\echocast_local_pilot.db
  demo_codes_present: []
...
EchoCast pilot process before stopping it, and clears the pilot variables from
...
| `the protected EchoCast database was refused` | A path resolved to `backend\echocast_live.db`. This is the safety net working. |
...
| Stale PID file | `Stop-EchoCastLocalPilot.ps1` detects and removes it safely. |
...
Never delete the repository, and never delete `backend\echocast_live.db`, its
