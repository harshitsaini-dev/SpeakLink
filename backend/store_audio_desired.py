...
So DESIRED STATE is now a first-class, always-present fact about a Store, and
...
THE THREE FACTS, WHICH ARE NEVER MERGED
    ...
    DESIRED     what HQ wants        - persisted here, set by an operator
...
those. "Applied 70%" is DESIRED wearing ACTUAL's clothes. A disabled slider is
CONNECTION deciding what DESIRED is allowed to be.
...
DESIRED_TABLE = "store_audio_desired_state"
...
#: How the DESIRED state relates to the ACTUAL one. Derived, never stored - a
...
SYNC_NO_DESIRED = "NO_DESIRED_STATE"


class StoreAudioDesiredError(RuntimeError):
    """Base class, so no caller handles one failure and misses another."""


class EmptyDesiredStateError(StoreAudioDesiredError):
    """Neither a volume nor a mute was asked for."""


class InvalidDesiredVolumeError(StoreAudioDesiredError):
    def __init__(self, value: object) -> None:
        super().__init__(f"volume must be an integer 0-100, got {value!r}")


@dataclass(frozen=True)
class DesiredAudioState:
    store_id: int
    device_id: int
    ...
    created_by: int | None
    updated_at: str
    #: Set only when an attempt to apply this desired state actually failed.
...
def ensure_desired_audio_schema(engine: Engine) -> None:
    ...
    with engine.begin() as connection:
        connection.exec_driver_sql(
            f"""
            CREATE TABLE IF NOT EXISTS {DESIRED_TABLE} (
                ...
                CONSTRAINT fk_desired_audio_store
                    FOREIGN KEY (store_id) REFERENCES stores(id) ON DELETE CASCADE,
                CONSTRAINT fk_desired_audio_device
                    FOREIGN KEY (device_id) REFERENCES receiver_devices(id)
                    ON DELETE CASCADE,
                CONSTRAINT fk_desired_audio_actor
                    FOREIGN KEY (created_by) REFERENCES hq_users(id) ON DELETE SET NULL,
                CONSTRAINT ck_desired_audio_volume CHECK (
                    volume_percent IS NULL
                    OR (volume_percent >= 0 AND volume_percent <= 100)
                ),
                -- A row that asks for nothing would render as a desired state
                -- that desires nothing.
                CONSTRAINT ck_desired_audio_not_empty CHECK (
                    volume_percent IS NOT NULL OR muted IS NOT NULL
                ),
                CONSTRAINT ck_desired_audio_utc CHECK (
        ...
        connection.exec_driver_sql(
            f"CREATE INDEX IF NOT EXISTS ix_{DESIRED_TABLE}_device "
            f"ON {DESIRED_TABLE}(device_id)"
        )
        ...
        if legacy:
            connection.exec_driver_sql(
                f"""
                INSERT OR IGNORE INTO {DESIRED_TABLE}
...
def _validate_volume(value: object) -> int | None:
    ...
    if isinstance(value, bool) or not isinstance(value, int):
        raise InvalidDesiredVolumeError(value)
    if not 0 <= value <= 100:
        raise InvalidDesiredVolumeError(value)
    return value


def set_desired(engine: Engine, *, store_id: int, device_id: int,
                volume_percent: int | None = None, muted: bool | None = None,
                created_by: int | None = None) -> DesiredAudioState:
    ...
    if volume_percent is None and muted is None:
        raise EmptyDesiredStateError(
            "a desired state must request a volume, a mute, or both")

    existing = get_desired(engine, store_id=store_id)
...
_SELECT = (
    "SELECT store_id, device_id, volume_percent, muted, created_by, "
    f"updated_at, last_error FROM {DESIRED_TABLE}"
)


def _row_to_state(row) -> DesiredAudioState:
    return DesiredAudioState(
...
def get_desired(engine: Engine, *, store_id: int) -> DesiredAudioState | None:
...
def all_desired(engine: Engine) -> dict[int, DesiredAudioState]:
    with engine.connect() as connection:
        rows = connection.execute(text(_SELECT)).fetchall()
    return {row[0]: _row_to_state(row) for row in rows}


def clear_desired(engine: Engine, *, store_id: int) -> bool:
    """Forget what HQ wanted, leaving the Store to whatever it is doing.

    Deliberately NOT called after a successful apply. Desired state is a
    ...
    with engine.begin() as connection:
        result = connection.execute(
            text(f"DELETE FROM {DESIRED_TABLE} WHERE store_id = :store_id"),
            {"store_id": store_id})
    return bool(result.rowcount)


def mark_sync_failed(engine: Engine, *, store_id: int, error: str) -> None:
    """Record why the desired state could not be reached.

    The desired state itself SURVIVES. What the operator asked for has not
    ...
    with engine.begin() as connection:
        connection.execute(
            text(f"UPDATE {DESIRED_TABLE} SET last_error = :error "
...
def clear_sync_error(engine: Engine, *, store_id: int) -> None:
    with engine.begin() as connection:
        connection.execute(
            text(f"UPDATE {DESIRED_TABLE} SET last_error = NULL "
                 "WHERE store_id = :store_id"),
            {"store_id": store_id})
...
def sync_state(*, desired, actual_volume_percent, actual_muted, online: bool,
    ...
    """How DESIRED currently relates to ACTUAL. Derived on every read.
