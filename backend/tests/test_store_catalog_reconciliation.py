...
open, copy, migrate, seed or modify ``backend/speaklink_live.db``, start
...
``SPEAKLINK_DB_PATH`` said at import time and installs a ``connect`` listener
...
_PROTECTED = Path(__file__).resolve().parents[1] / "speaklink_live.db"
_configured = os.environ.get("SPEAKLINK_DB_PATH")
if not _configured or Path(_configured).resolve() == _PROTECTED.resolve():
    os.environ["SPEAKLINK_DB_PATH"] = str(
        Path(tempfile.gettempdir())
        / f"speaklink-reconciliation-{os.environ.get('PYTEST_XDIST_WORKER', 'serial')}.db"
    )
...
def test_relative_and_dotdot_protected_paths_are_refused():
    before = _real_database_metadata()
    dotdot = PROTECTED_DATABASE_PATH.parent / ".." / "backend" / "speaklink_live.db"
    with pytest.raises(ProtectedDatabaseError):
        reconcile(dotdot)
    with pytest.raises(ProtectedDatabaseError):
