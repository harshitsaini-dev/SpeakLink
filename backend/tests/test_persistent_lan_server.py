...
``Start-EchoCastLanPilot.ps1`` line 92 builds its data root from the clock::
...
def test_the_root_is_fixed_not_timestamped(monkeypatch):
    ...
    assert first == second
    assert first == Path(r"C:\Users\someone\AppData\Local\EchoCast-AI\persistent-lan-server")
...
def test_the_layout_is_stable(monkeypatch, tmp_path):
    ...
    assert profile.database == resolve_persistent_root() / "data" / "echocast.db"
...
def test_the_protected_database_is_not_a_throwaway():
    ...
    disposable."""
    assert not is_throwaway_pilot_database(BACKEND_ROOT / "echocast_live.db")
...
def test_the_description_names_no_secret(monkeypatch, tmp_path):
    ...
    for forbidden in ("JWT_SECRET", "password", "echocast_rcv_v1", "Bearer"):
...
@pytest.mark.parametrize("name", [
    ...
    "Start-EchoCastPersistentLanServer.ps1",
    "Stop-EchoCastPersistentLanServer.ps1",
...
def test_initialize_defaults_to_planonly():
    ...
    source = (SCRIPTS / "Initialize-EchoCastPersistentLanServer.ps1").read_text(encoding="utf-8")
...
def test_start_never_builds_a_dated_path():
    ...
    source = (SCRIPTS / "Start-EchoCastPersistentLanServer.ps1").read_text(encoding="utf-8")
...
def test_no_script_carries_a_password_or_secret():
    ...
    The first version failed on Test-EchoCastPersistentLanServer.ps1, whose
    whole job includes a regex that detects 'echocast_rcv_v1' in a lock file. A
    ...
    for path in SCRIPTS.glob("*EchoCastPersistentLanServer.ps1"):
        ...
        for line in source.splitlines():
            if "-match" in line or "-notmatch" in line or line.strip().startswith("#"):
                continue
            assert not re.search(r"echocast_rcv_v1\.[A-Za-z0-9]", line), f"{path.name}: {line.strip()}"
