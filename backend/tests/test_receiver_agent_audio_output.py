...
returns the *null* sink unless ``SPEAKLINK_AUDIO_SINK_MODE`` says otherwise, and
...
def test_the_default_sink_discards_audio(monkeypatch):
    ...
    monkeypatch.delenv("SPEAKLINK_AUDIO_SINK_MODE", raising=False)
    monkeypatch.delenv("SPEAKLINK_AUDIO_OUTPUT_DEVICE", raising=False)
...
def test_a_leftover_device_variable_cannot_cause_a_sound(monkeypatch):
    monkeypatch.delenv("SPEAKLINK_AUDIO_SINK_MODE", raising=False)
    monkeypatch.setenv("SPEAKLINK_AUDIO_OUTPUT_DEVICE", "index:5")
...
def test_the_description_never_contains_a_credential():
    for configuration in (
        ...
        joined = " ".join(describe_sink(configuration))
        assert "speaklink_rcv_v1" not in joined
...
def test_the_refusal_names_no_secret(capsys, tmp_path):
    ...
    assert "speaklink_rcv_v1" not in output
