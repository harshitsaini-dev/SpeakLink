...
LAUNCHER = TOOLS / "echocast_server.py"
...
def launcher():
    import importlib
    module = importlib.import_module("echocast_server")
...
def default_data_environment(monkeypatch):
    ...
    The suite sets ECHOCAST_DATA_DIR globally so a test that starts a broadcast
    ...
    """
    monkeypatch.delenv("ECHOCAST_DATA_DIR", raising=False)
...
def test_data_lives_inside_the_repository_by_default(launcher, default_data_environment):
    ...
    assert launcher.database_path() == REPOSITORY_ROOT / "data" / "echocast.db"
...
def test_no_developer_path_or_lan_address_is_hardcoded(launcher):
    for forbidden in ("C:\\Users\\", "192.168.4.134", "Desktop\\EchoCast"):
...
def launcher_commands():
    import importlib
    return importlib.import_module("echocast_server").COMMANDS
