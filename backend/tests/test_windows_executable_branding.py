"""Every shipped EchoCast executable carries the EchoCast icon.

WHY THIS TEST EXISTS

PyInstaller's default icon is a Python logo. Shipping it means a Store PC
desktop, a taskbar and a Start Menu entry that do not say EchoCast - and
...
CANONICAL_ICON = REPOSITORY_ROOT / "assets" / "echocast.ico"
...
def test_the_canonical_icon_exists():
    assert CANONICAL_ICON.exists(), (
        "assets/echocast.ico is missing - run tools/build_windows_icon.py")
...
def test_no_spec_hard_codes_a_developer_path(spec_name):
    ...
    source = (REPOSITORY_ROOT / spec_name).read_text(encoding="utf-8")
    for forbidden in ("C:\\Users\\", "C:/Users/", "/home/", "Desktop\\EchoCast"):
...
def test_every_shipped_spec_points_at_the_one_canonical_icon(spec_name):
    ...
    until somebody notices two EchoCast windows with different logos."""
    source = (REPOSITORY_ROOT / spec_name).read_text(encoding="utf-8")
    assert 'assets" / "echocast.ico"' in source or "assets/echocast.ico" in source, (
        f"{spec_name} does not use assets/echocast.ico")
...
def test_built_executables_carry_an_icon_resource():
    ...
    for executable in _built_executables():
        ...
        assert biggest in raw, (
            f"{executable.name} does not contain the EchoCast icon image")
