...
copy, migrate or modify ``backend/speaklink_live.db``, and they never create
Receiver Devices or production credentials.

That was true of every *test* here and still not enough. Importing ``db`` binds
a process-wide engine to whatever ``SPEAKLINK_DB_PATH`` said at import time, and
...
if not _configured or Path(_configured).resolve() == _PROTECTED.resolve():
    os.environ["SPEAKLINK_DB_PATH"] = str(
        ...
        / f"speaklink-catalog-{os.environ.get('PYTEST_XDIST_WORKER', 'serial')}.db"
    )
...
def test_catalog_contains_no_credential_or_token_material():
    for entry in CANONICAL_STORES:
        ...
        for marker in ("token", "secret", "password", "bearer", "speaklink_rcv", "hmac"):
