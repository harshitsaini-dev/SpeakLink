...
def _synthetic_package(artifacts_root: Path, *, background_subsystem=2) -> Path:
    package = artifacts_root / "SpeakLinkReceiver-9.9.9-e2e0000-20260101-000000"
    package.mkdir(parents=True)
    _fake_pe(package / "SpeakLinkReceiver.exe", 3)
...
class _RecordingInstaller:
    """Stands in for Install-SpeakLinkStoreReceiver.ps1. Records the whole
...
def test_the_full_first_run_chain_reaches_connected(fresh, tmp_path):
    ...
    for command in installer.commands:
        ...
        assert "speaklink_rcv_v1" not in joined, "a credential reached a command line"
    config_text = config_path.read_text(encoding="utf-8")
    assert code not in config_text
    assert "speaklink_rcv_v1" not in config_text
    status_text = status_path.read_text(encoding="utf-8")
    assert "speaklink_rcv_v1" not in status_text
...
def test_the_protected_database_was_never_opened(fresh):
    ...
    assert "speaklink_live" not in str(fresh.db.DB_PATH)
