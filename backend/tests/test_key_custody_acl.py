...
can open it. That is the decision recorded for SpeakLink: one Windows server, one
dedicated local account `SpeakLinkService`, CURRENT_USER scope under that account,
and the container at `C:\\ProgramData\\SpeakLink\\keys\\receiver-hmac-keys.bin`.
...
`SpeakLinkService` reads and writes, Administrators keep recovery access, and
...
HARDENED = r"""C:\ProgramData\SpeakLink\keys\receiver-hmac-keys.bin NT AUTHORITY\SYSTEM:(F)
                                                                    BUILTIN\Administrators:(F)
                                                                    HARSHIT-DESKTOP\SpeakLinkService:(R,W)

Successfully processed 1 files; Failed processing 0 files"""

INHERITED_FROM_PROGRAMDATA = r"""C:\ProgramData\SpeakLink\keys\receiver-hmac-keys.bin NT AUTHORITY\SYSTEM:(I)(F)
...
MISSING_SERVICE = r"""C:\ProgramData\SpeakLink\keys\receiver-hmac-keys.bin NT AUTHORITY\SYSTEM:(F)
                                                                    BUILTIN\Administrators:(F)

Successfully processed 1 files; Failed processing 0 files"""

EVERYONE_CAN_READ = r"""C:\ProgramData\SpeakLink\keys\receiver-hmac-keys.bin NT AUTHORITY\SYSTEM:(F)
                                                                    BUILTIN\Administrators:(F)
                                                                    Everyone:(R)
                                                                    HARSHIT-DESKTOP\SpeakLinkService:(R,W)
...
SERVICE_HAS_FULL = r"""C:\ProgramData\SpeakLink\keys\receiver-hmac-keys.bin NT AUTHORITY\SYSTEM:(F)
                                                                    BUILTIN\Administrators:(F)
                                                                    HARSHIT-DESKTOP\SpeakLinkService:(F)

Successfully processed 1 files; Failed processing 0 files"""


def _verdict(output: str, account: str = "HARSHIT-DESKTOP\\SpeakLinkService") -> AclVerdict:
...
def test_a_hardened_acl_parses_into_its_three_principals():
    ...
    assert set(entries) == {
        ...
        "HARSHIT-DESKTOP\\SpeakLinkService",
    }
    assert entries["HARSHIT-DESKTOP\\SpeakLinkService"].rights == {"R", "W"}
...
def test_inherited_entries_are_recognised_as_inherited():
    ...
    assert entries["HARSHIT-DESKTOP\\SpeakLinkService"].inherited is False
...
def test_a_missing_service_account_is_rejected():
    ...
    assert verdict.acceptable is False
    assert any("SpeakLinkService" in problem for problem in verdict.problems)
...
def test_the_grant_plan_breaks_inheritance_before_granting():
    ...
    plan = icacls_grant_plan(SERVICE_CONTAINER_PATH, service_account="HOST\\SpeakLinkService")
    joined = " || ".join(plan)
    assert plan[0].count("/inheritance:r") == 1, "inheritance is not broken first"
    assert joined.index("/inheritance:r") < joined.index("SpeakLinkService")


def test_the_grant_plan_gives_the_service_account_only_read_and_write():
    plan = " ".join(icacls_grant_plan(SERVICE_CONTAINER_PATH, service_account="HOST\\SpeakLinkService"))
    assert "SpeakLinkService:(R,W)" in plan
    assert "SpeakLinkService:(F)" not in plan
...
def test_the_plan_quotes_the_path_because_it_contains_no_spaces_today_but_might():
    plan = icacls_grant_plan(Path(r"C:\Program Files\SpeakLink\keys\k.bin"), service_account="HOST\\Svc")
    assert all('"C:\\Program Files\\SpeakLink\\keys\\k.bin"' in step for step in plan)
...
def test_the_container_path_matches_the_architecture_decision():
    assert SERVICE_CONTAINER_PATH == Path(r"C:\ProgramData\SpeakLink\keys\receiver-hmac-keys.bin")
...
def test_the_service_account_name_is_the_agreed_one():
    assert SERVICE_ACCOUNT == "SpeakLinkService"
