...
never open, copy or modify ``backend/echocast_live.db``.
...
def audio_pilot_result(tmp_path_factory):
    ...
    root = tmp_path_factory.mktemp("echocast-audio") / "local-pilot"
    previous = {
        key: os.environ.get(key)
        for key in (ADMIN_USERNAME_ENV, ADMIN_PASSWORD_ENV, "ECHOCAST_DB_PATH", "JWT_SECRET")
    }
    ...
    os.environ.pop("ECHOCAST_DB_PATH", None)
