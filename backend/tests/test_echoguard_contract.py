"""Contract tests for an EchoGuard integration that does not exist yet.

A full search of this repository finds EchoGuard mentioned 21 times, every one
...
behaves the way a real one must. When the real EchoGuard interface arrives, an
...
from echoguard import (  # noqa: E402
...
def test_no_pause_or_resume_interface_exists_to_integrate_with():
    ...
    for path in REPOSITORY_ROOT.rglob("*.py"):
        if any(part in {"node_modules", ".venv", ".git"} for part in path.parts):
            continue
        if path.name in {"echoguard.py", "test_echoguard_contract.py"}:
            continue
        text = path.read_text(encoding="utf-8", errors="ignore")
        for match in re.finditer(r"(?i)echoguard[_.]?(pause|resume|client|service|process)", text):
            line = text[: match.start()].count("\n") + 1
            callers.append(f"{path.relative_to(REPOSITORY_ROOT)}:{line}")

    assert not callers, (
        "something already talks to an EchoGuard pause/resume surface: "
        f"{callers}. Replace NullEchoGuard with a real adapter."
    )


def test_the_acoustic_verification_message_contract_already_exists():
    """Corrects an earlier claim that no EchoGuard contract existed at all.

    receiver_contract.py defines TrustedSpeakerVerifiedEvent with
    source: Literal["echoguard"]. It is a schema, not an implementation - but it
    is the shape a real EchoGuard must speak, and it was already decided.
    """
...
def test_the_null_adapter_satisfies_the_protocol():
    assert isinstance(NullEchoGuard(), EchoGuardAdapter)
...
def test_a_pause_must_be_acknowledged_not_merely_attempted():
    guard = FakeEchoGuard(acknowledge=False)
...
def test_a_refusal_is_reported_as_a_refusal():
    guard = FakeEchoGuard(refuse=True)
    result = guard.pause(session_id=1, timeout_seconds=5)
...
def test_a_fake_adapter_satisfies_the_protocol():
    assert isinstance(FakeEchoGuard(), EchoGuardAdapter)
...
# ---------------------------------------------------------------------------
# EchoGuard is the only thing that may ever produce SPEAKER_VERIFIED
# ---------------------------------------------------------------------------
...
def test_nothing_in_this_module_claims_speaker_verification():
    source = (BACKEND_ROOT / "echoguard.py").read_text(encoding="utf-8")
