"""The logic behind EchoCastStoreSetup.exe. No GUI import here, on purpose.
...
def test_a_public_https_url_is_accepted_and_checked():
    def opener(url, timeout):
        ...
        return _FakeResponse(200, {"service": "EchoCast Live", "status": "ok"})
...
def test_a_public_http_url_is_refused_before_any_network_call():
    calls = []
    result = core.test_hq_connection("http://echocast.example.com",
...
def test_a_reachable_but_wrong_service_is_connection_failed():
    result = core.test_hq_connection(
        ...
        opener=lambda *a, **k: _FakeResponse(200, {"status": "not-echocast"}),
    )
    assert result.state is core.ConnectionState.CONNECTION_FAILED
    ...
    class _FakeTransport:
        def post_json(self, url, payload, *, timeout):
            return 201, {"device_public_id": "dev-123", "store_id": 7,
                        "credential": "echocast_rcv_v1.11111111-1111-1111-1111-111111111111.AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"}
...
def test_test_sound_never_claims_speaker_verified(monkeypatch):
    """SPEAKER_VERIFIED is reserved for EchoGuard acoustic evidence."""
