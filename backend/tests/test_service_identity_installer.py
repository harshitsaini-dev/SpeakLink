...
* it hardened only the key directory. The application, data and log directories
  under ``C:\\ProgramData\\EchoCast-AI`` were left inheriting ``BUILTIN\\Users``.
...
INSTALLER = SCRIPTS / "Install-EchoCastServiceIdentity.ps1"
SERVICE = "HOST\\EchoCastService"
...
HARDENED_RW = _acl(
    ...
    "HOST\\EchoCastService:(OI)(CI)(R,W)",
)
HARDENED_RX = _acl(
    "NT AUTHORITY\\SYSTEM:(OI)(CI)(F)",
    "BUILTIN\\Administrators:(OI)(CI)(F)",
    "HOST\\EchoCastService:(OI)(CI)(RX)",
)
WITH_USERS = _acl(
...
def test_the_four_directories_are_the_agreed_ones():
    assert APPLICATION_DIRECTORY == Path(r"C:\ProgramData\EchoCast-AI\app")
    assert KEY_DIRECTORY == Path(r"C:\ProgramData\EchoCast-AI\keys")
    assert DATA_DIRECTORY == Path(r"C:\ProgramData\EchoCast-AI\data")
    assert LOG_DIRECTORY == Path(r"C:\ProgramData\EchoCast-AI\logs")
...
def test_acls_are_verified_rather_than_assumed(installer_code: str):
    ...
    assert "Test-EchoCastDirectoryAcl -Path" in installer_code, (
