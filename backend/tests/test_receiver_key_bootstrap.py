...
* ``scripts/Test-EchoCastHQAutoStart.ps1`` - *"the backend mints the HMAC
...
def profile(tmp_path: Path):
    ...
    database = _database(tmp_path / "echocast.db")
...
def test_an_existing_corrupt_container_refuses_rather_than_replacing_it(profile):
    ...
    container, database = profile
    container.write_bytes(b"ECHOCAST-KEYS-v1\x00garbage that will not decode")
    fingerprint = _fingerprint(container)
...
def test_the_hq_package_ships_the_bootstrap_module():
    ...
    build_script = (REPOSITORY_ROOT / "scripts" / "Build-EchoCastHQPackage.ps1").read_text(
        encoding="utf-8"
    )
...
def test_an_unconfigured_process_attempts_nothing(monkeypatch, tmp_path):
    """The first version of this gated on ECHOCAST_DB_PATH alone, and it would
    have had THIS SUITE mint a live container under C:\\ProgramData: conftest
    always sets ECHOCAST_DB_PATH, the temporary database has zero Devices, and the
    ...
    monkeypatch.delenv("ECHOCAST_KEY_CONTAINER", raising=False)
    monkeypatch.setenv("ECHOCAST_DB_PATH", str(_database(tmp_path / "e.db")))
...
def test_a_managed_start_with_both_variables_creates(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ECHOCAST_KEY_CONTAINER", str(container))
    monkeypatch.setenv("ECHOCAST_DB_PATH", str(_database(tmp_path / "e.db")))
...
def test_the_real_service_container_path_is_never_the_implicit_target():
    ...
    assert "ECHOCAST_KEY_CONTAINER" in literals, (
...
def test_a_real_backend_import_mints_the_container_and_loads_the_ring(tmp_path):
    ...
    program = (
        ...
        "path = os.environ['ECHOCAST_KEY_CONTAINER'];"
...
def test_a_real_backend_start_refuses_when_devices_are_enrolled(tmp_path):
    ...
    keys.mkdir()
    container = keys / "receiver-hmac-keys.bin"
    database = _database(tmp_path / "echocast.db", devices=3)
    environment = dict(
        os.environ,
        ECHOCAST_DB_PATH=str(database),
        ECHOCAST_KEY_CONTAINER=str(container),
