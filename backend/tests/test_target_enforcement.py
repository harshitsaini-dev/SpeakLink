...
mouse. They drag the slider, SpeakLink snaps it back, they drag it again - and
because an endpoint notification arrives within milliseconds, that loop would
run as fast as the socket allows.

So every test here is about a bound: how long SpeakLink waits, how long it then
...
def test_repeated_slider_activity_cannot_produce_unbounded_commands(policy):
    """The whole hazard, simulated: somebody fighting SpeakLink for an hour."""
    commands = 0
    now = 0.0
