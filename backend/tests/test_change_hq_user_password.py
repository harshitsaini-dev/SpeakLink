...
os.environ.setdefault("SPEAKLINK_DB_PATH", str(Path(os.environ.get("TEMP", "/tmp")) /
                                              "speaklink-tests-default-engine.db"))
...
def test_the_two_targets_resolve_to_exactly_the_documented_files():
    ...
    assert resolve_target(TARGET_PROTECTED) == BACKEND_ROOT / "speaklink_live.db"
...
def test_no_password_is_read_from_the_environment():
    source = TOOL.read_text(encoding="utf-8")
    for forbidden in ("ADMIN_PASSWORD", "SPEAKLINK_PASSWORD", "getenv(\"PASS"):
...
def test_the_tool_refuses_while_an_hq_process_is_running():
    source = TOOL.read_text(encoding="utf-8")
    assert "SpeakLinkHQRuntime" in source, "nothing checks for a running HQ runtime"
