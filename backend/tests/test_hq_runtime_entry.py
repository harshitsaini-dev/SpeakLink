...
``tools/hq_runtime.py`` was built into ``EchoCastHQRuntime.exe`` and the
...
def persistent(tmp_path, monkeypatch):
    ...
    root = tmp_path / "EchoCast-AI" / "persistent-lan-server"
    for folder in ("data", "config", "keys", "logs", "backups", "runtime",
                   "migration-reports"):
        (root / folder).mkdir(parents=True)
    (root / "data" / "echocast.db").write_bytes(b"SQLite format 3\x00")
...
def test_the_status_file_records_state_and_no_secret(tmp_path):
    ...
    write_status(status, RuntimeState.READY,
                 detail="backend healthy; token echocast_rcv_v1.abc.def")
    payload = read_status(status)
    assert payload["state"] == "READY"
    assert "echocast_rcv_v1" not in json.dumps(payload)
...
def test_degraded_writes_a_safe_reason(persistent, monkeypatch):
    ...
    for forbidden in ("not-a-real-secret", "password", "echocast_rcv_v1"):
...
#
# EchoCastHQRuntime.exe --check against the real initialized persistent root
...
# Start-EchoCastPersistentLanServer.ps1 mints the signing secret. So the
...
def _database_with_devices(root: Path, count: int) -> None:
    ...
    database = root / "data" / "echocast.db"
...
def test_an_unreadable_database_is_refused_rather_than_assumed_empty(
    ...
    monkeypatch.setattr(hq_runtime, "FRONTEND_BUILD", _fake_build(persistent))
    (persistent / "keys" / "receiver-hmac-keys.bin").unlink()
    (persistent / "data" / "echocast.db").write_bytes(b"this is not a database")
...
def test_a_missing_signing_secret_is_created_not_refused(persistent, monkeypatch):
    ...
    re-enrolment of 44 Stores. Start-EchoCastPersistentLanServer.ps1 already
...
# <dist>\EchoCastHQRuntime\frontend\build and refused. Correct refusal, wrong
...
def test_the_packaged_layout_looks_beside_the_executable(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, "executable", str(tmp_path / "EchoCastHQRuntime.exe"))
...
def test_the_installer_and_the_runtime_agree_on_where_the_frontend_lives():
    ...
    installer = (REPOSITORY_ROOT / "scripts" /
                 "Install-EchoCastHQAutoStart.ps1").read_text(encoding="utf-8")
...
def test_help_renders(persistent, monkeypatch, capsys):
    ...
    with pytest.raises(SystemExit):
        main(["--help"])
    assert "EchoCastHQRuntime" in capsys.readouterr().out
