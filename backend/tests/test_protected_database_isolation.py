...
``backend/echocast_live.db``. The main file's bytes never changed, so every
...
def _load_conftest():
    ...
    spec = importlib.util.spec_from_file_location(
        "echocast_tests_conftest", BACKEND_ROOT / "tests" / "conftest.py"
    )
...
def _import_module_alone(module_name: str, *, inherited_db_path: str | None) -> str:
    ...
    environment = {
        key: value for key, value in os.environ.items() if key != "ECHOCAST_DB_PATH"
    }
    if inherited_db_path is not None:
        environment["ECHOCAST_DB_PATH"] = inherited_db_path
...
def test_the_conftest_replaces_an_inherited_protected_path():
    environment = {**os.environ, "ECHOCAST_DB_PATH": str(PROTECTED_DATABASE)}
    completed = subprocess.run(
