...
of them open, copy, migrate, seed or modify ``backend/speaklink_live.db``.

The harness exists to make SpeakLink ready for *local software* pilot testing.
...
def pilot_root(tmp_path):
    ...
    return tmp_path / "SpeakLink" / "local-pilot"
...
def test_relative_and_dotdot_protected_paths_are_refused(monkeypatch):
    ...
    with pytest.raises(ProtectedPathError):
        reject_protected_database(Path("speaklink_live.db"))
    with pytest.raises(ProtectedPathError):
        reject_protected_database(
            PROTECTED_DATABASE.parent / ".." / "backend" / "speaklink_live.db"
        )
    assert _protected_metadata() == before
...
def test_pilot_database_path_is_never_the_protected_filename_in_backend(tmp_path):
    ...
    assert paths.database_path.name != "speaklink_live.db"
...
def smoke_result(tmp_path_factory):
    ...
    root = tmp_path_factory.mktemp("speaklink-pilot") / "local-pilot"
    previous = {
