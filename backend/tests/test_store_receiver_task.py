...
secret in the XML - was verified only by Test-EchoCastStoreReceiver.ps1, which
...
INSTALL = SCRIPTS / "Install-EchoCastStoreReceiver.ps1"
VERIFY = SCRIPTS / "Test-EchoCastStoreReceiver.ps1"
REPAIR = SCRIPTS / "Repair-EchoCastStoreReceiver.ps1"
UNINSTALL = SCRIPTS / "Uninstall-EchoCastStoreReceiver.ps1"
...
def test_the_task_runs_the_windowed_executable():
    ...
    assert "EchoCastReceiverBackground.exe" in action


def test_the_task_never_runs_the_console_executable():
    action = _task_action_block(_text(INSTALL))
    assert not re.search(r"EchoCastReceiver\.exe", action), (
...
def test_repair_delegates_hashing_to_the_installer_rather_than_repeating_it():
    ...
    settings and re-runs Install-EchoCastStoreReceiver.ps1 on top of them, so
    ...
    body = _text(REPAIR)
    assert "Install-EchoCastStoreReceiver.ps1" in body
