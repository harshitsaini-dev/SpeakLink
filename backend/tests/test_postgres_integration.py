...
SpeakLink actually performs - creating a User, editing a Store, tombstoning
...
Every test runs inside a freshly generated ``speaklink_test_*`` schema and
...
@pytest.fixture()
def pg(pg_engine):
    """An isolated PostgreSQL schema with the full SpeakLink schema created."""
...
def test_receiver_credential_uniqueness_and_revocation_rule(pg):
    ...
    with pg.begin() as c:
        ...
        c.execute(text(
            ...
            "VALUES (:p, :d, 1, 'speaklink_rcv', :h, 1, 'active', :now, :now)"),
    ...
    with pytest.raises(IntegrityError):
        with pg.begin() as c:
            c.execute(text(
                "INSERT INTO receiver_credentials (public_id, device_id, credential_version, "
                "token_format, token_hash, hash_key_version, status, issued_at, created_at) "
                "VALUES (:p, :d, 2, 'speaklink_rcv', :h, 1, 'revoked', :now, :now)"),
...
def test_timestamps_round_trip_without_losing_utc(pg):
    """SpeakLink stores UTC everywhere. A dialect change must not quietly
