...
def test_a_fresh_computer_opens_on_welcome(tmp_path):
    ...
    try:
        ...
        assert "EchoCast Store Receiver Setup" in shown
...
def test_the_old_enrolment_screen_says_what_happened_in_plain_words(tmp_path, state_root):
    ...
    try:
        shown = _texts(app._current)
        assert "Old EchoCast enrolment detected" in shown
        assert "earlier EchoCast pilot server" in shown or "previous EchoCast pilot server" in shown
