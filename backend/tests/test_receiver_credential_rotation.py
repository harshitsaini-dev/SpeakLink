...
``backend/speaklink_live.db`` and the real pilot database are never opened.
"""
...
def test_rotation_issues_exactly_one_new_credential(runtime: Runtime):
    ...
    assert credential.startswith("speaklink_rcv_v")
...
def test_rotation_writes_a_credential_event(runtime: Runtime):
    ...
    assert "speaklink_rcv_v" not in (metadata or "")
