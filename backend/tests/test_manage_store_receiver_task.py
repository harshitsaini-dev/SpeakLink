"""Manage-EchoCastStoreReceiverTask.ps1: start/stop/status for one named task,
never anything else.
...
task's own action names EchoCastReceiverBackground.exe.
"""
...
SCRIPT = REPOSITORY_ROOT / "scripts" / "Manage-EchoCastStoreReceiverTask.ps1"
...
def test_it_verifies_ownership_before_starting_or_stopping():
    body = _text()
    assert "EchoCastReceiverBackground.exe" in body
...
ISOLATED_TASK = "EchoCast Store Receiver (automated test - do not keep)"
...
def test_start_refuses_a_task_that_is_not_ours(tmp_path):
    ...
    try:
        ...
        assert "not ours" in (result.stdout + result.stderr).lower() or \
               "EchoCastReceiverBackground" in (result.stdout + result.stderr)
    finally:
