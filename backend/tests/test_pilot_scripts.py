...
# Stop-EchoCastLocalPilot.ps1 stopped only the PID it recorded at launch - the
...
# ---------------------------------------------------------------------------
STOP_SCRIPTS = (
    "Stop-EchoCastLocalPilot.ps1",
    "Stop-EchoCastAudioReceiverPilot.ps1",
    "Stop-EchoCastWindowsAudioReceiverPilot.ps1",
)
...
def test_stop_scripts_use_the_tested_process_tree_planner(name: str):
    ...
    assert script.exists(), f"{name} is missing"
    text = script.read_text(encoding="utf-8")
    assert "Stop-EchoCastProcessTree" in text, (
    ...
    assert "EchoCastProcessTree.ps1" in text, f"{name} does not dot-source the shared helper"
...
def test_the_local_pilot_stop_script_verifies_both_ports_are_released():
    ...
    text = (SCRIPTS_DIRECTORY / "Stop-EchoCastLocalPilot.ps1").read_text(encoding="utf-8")
    assert "Test-EchoCastPortReleased -Port 8000" in text, "port 8000 is never verified"
    assert "Test-EchoCastPortReleased -Port 3000" in text, "port 3000 is never verified"


def test_the_shared_helper_removes_a_pid_file_only_after_the_tree_is_gone():
    text = (SCRIPTS_DIRECTORY / "EchoCastProcessTree.ps1").read_text(encoding="utf-8")
...
def test_the_shared_helper_never_uses_write_output_inside_its_functions():
    ...
    value. A single Write-Output inside Stop-EchoCastProcessTree turns
    ...
    to enforce. Caught by running the real stop path: Test-EchoCastPortReleased
...
@pytest.mark.parametrize("function", ["Stop-EchoCastProcessTree", "Test-EchoCastPortReleased"])
...
def test_start_scripts_record_the_creation_time_for_pid_reuse_detection():
    ...
    for name in (
        ...
        text = (SCRIPTS_DIRECTORY / name).read_text(encoding="utf-8")
        assert "Write-EchoCastPidFile" in text, (
