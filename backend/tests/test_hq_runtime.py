...
"helpful" fallback - create the database, use backend/echocast_live.db, pick a
...
def test_a_complete_profile_resolves(persistent):
    ...
    assert profile.database == root / "data" / "echocast.db"
...
def test_a_missing_database_is_refused_and_never_created(persistent):
    root, _build = persistent
    (root / "data" / "echocast.db").unlink()
    with pytest.raises(RuntimeConfigError):
        resolve_runtime_profile()
    assert not (root / "data" / "echocast.db").exists(), "it created one anyway"
...
def test_it_never_falls_back_to_the_repository_database():
    ...
    offenders = [
        ...
        and node.value not in docstrings and "echocast_live.db" in node.value
    ]
...
def test_the_degraded_outcome_carries_a_safe_reason():
    ...
    for forbidden in ("password", "secret", "echocast_rcv_v1"):
...
def test_the_spec_builds_a_windowed_runtime():
    ...
    assert "EchoCastHQRuntime" in spec
