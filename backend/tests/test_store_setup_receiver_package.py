...
nothing. This locates the newest SpeakLinkReceiver-* package built by
Build-SpeakLinkReceiver.ps1 and re-verifies it independently before StoreSetup
...
def _build_package(root: Path, name: str, *, console_subsystem=3, background_subsystem=2,
    ...
    _fake_pe(package / "SpeakLinkReceiver.exe", console_subsystem)
    _fake_pe(package / "SpeakLinkReceiverBackground.exe", background_subsystem)
    if with_ffmpeg:
    ...
    (package / "manifest.json").write_text(json.dumps({"product": "SpeakLinkReceiver"}),
                                           encoding="utf-8")
...
def test_a_correct_package_verifies(tmp_path):
    package = _build_package(tmp_path, "SpeakLinkReceiver-1.0.0-abc1234-20260101-000000")
...
def test_a_missing_background_exe_is_refused(tmp_path):
    ...
    (package / "SpeakLinkReceiverBackground.exe").unlink()
    verdict = core.verify_receiver_package(package)
    assert not verdict.ok
    assert any("SpeakLinkReceiverBackground.exe" in r for r in verdict.reasons)
...
def test_the_newest_valid_package_is_chosen(tmp_path):
    ...
    older = _build_package(tmp_path, "SpeakLinkReceiver-1.0.0-aaa1111-20260101-000000")
    time.sleep(0.05)
    newer = _build_package(tmp_path, "SpeakLinkReceiver-1.0.1-bbb2222-20260102-000000")
...
def test_a_newer_but_broken_package_is_skipped_for_an_older_good_one(tmp_path):
    ...
    good = _build_package(tmp_path, "SpeakLinkReceiver-1.0.0-aaa1111-20260101-000000")
    time.sleep(0.05)
    _build_package(tmp_path, "SpeakLinkReceiver-1.0.1-bbb2222-20260102-000000",
...
def test_when_every_candidate_is_broken_the_error_names_why(tmp_path):
    _build_package(tmp_path, "SpeakLinkReceiver-1.0.0-aaa1111-20260101-000000",
                   with_ffmpeg=False)
...
def test_non_receiver_directories_under_artifacts_are_ignored(tmp_path):
    (tmp_path / "SpeakLinkHQ-0.1.0-abc-20260101-000000").mkdir()
    good = _build_package(tmp_path, "SpeakLinkReceiver-1.0.0-aaa1111-20260101-000000")
    chosen = core.locate_verified_receiver_package(artifacts_root=tmp_path)
    assert chosen == good
