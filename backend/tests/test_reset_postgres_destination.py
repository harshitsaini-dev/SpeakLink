...
generated ``echocast_test_*`` schema every other PostgreSQL test uses, and is
...
def test_the_tool_knows_exactly_the_migration_tools_table_inventory():
    ...
    assert reset_tool._echocast_tables() == list(TABLE_ORDER)
...
def test_deletion_order_is_the_reverse_of_the_fk_safe_creation_order():
    ...
    order = reset_tool._echocast_tables()
...
def test_no_echocast_table_is_also_a_protected_schema_name():
    assert not (set(reset_tool._echocast_tables()) & reset_tool.PROTECTED_SCHEMAS)
...
def test_the_reset_is_one_transaction_so_a_failure_deletes_nothing(pg_engine, monkeypatch):
    ...
    real_order = reset_tool._echocast_tables()
    ...
    monkeypatch.setattr(reset_tool, "_echocast_tables", lambda: poisoned_order)
