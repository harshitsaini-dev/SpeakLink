...
def default_container_path() -> Path:
    ...
    return Path(base) / "EchoCast-AI" / "keys" / "receiver-hmac-keys.bin"
...
def _open(path: Path, protector: Protector) -> dict:
    if not path.exists():
        raise KeyContainerMissing(
            f"no Receiver key container at {path}. EchoCast will not invent one: "
    ...
    if len(raw) <= len(MAGIC) or not raw.startswith(MAGIC):
        raise KeyContainerCorrupt("that file is not an EchoCast key container")
