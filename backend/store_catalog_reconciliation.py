...
def _reject_protected_database(resolved: Path) -> None:
    ...
    if resolved == protected:
        raise ProtectedDatabaseError(
            "the protected EchoCast database was refused; supply an isolated snapshot"
        )
    try:
        if resolved.exists() and protected.exists() and os.path.samefile(resolved, protected):
            raise ProtectedDatabaseError(
                "the supplied path is the protected EchoCast database; "
...
def render_text(report: ReconciliationReport) -> str:
    lines: list[str] = []
    summary = report.summary()
    lines.append("EchoCast Store Catalog Reconciliation Report (read-only)")
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "canonical EchoCast Store catalog. Changes nothing."
        ),
    )
    parser.add_argument(
        "--database",
