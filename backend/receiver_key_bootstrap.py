...
first start - ``tools/hq_runtime.py``, ``scripts/Test-EchoCastHQAutoStart.ps1``,
...
logger = logging.getLogger("echocast.keys")
...
def bootstrap_from_environment(*, container_path, protector) -> "BootstrapOutcome | None":
    ...
    * ``ECHOCAST_KEY_CONTAINER`` - because the fallback is
      ``SERVICE_CONTAINER_PATH`` under ``C:\\ProgramData``, which is the machine's
      real service custody path. Gating on ``ECHOCAST_DB_PATH`` alone was the
      first version of this, and it would have had the **test suite** mint a live
      key container there: conftest always sets ``ECHOCAST_DB_PATH``, the
    ...
    * ``ECHOCAST_DB_PATH`` - because without it there is no database to count
      honestly, and creating against a guess is what every refusal above is for.

    ``tools/hq_runtime.py`` and ``Start-EchoCastPersistentLanServer.ps1`` both set
    the pair. A developer running the backend by hand sets neither and gets
    today's behaviour unchanged.
    """
    container = os.environ.get("ECHOCAST_KEY_CONTAINER")
    database = os.environ.get("ECHOCAST_DB_PATH")
