"""The contract LinkGuard must satisfy, and a no-op that satisfies nothing.

There is no LinkGuard **pause/resume** implementation in this repository - no
...
defines ``TrustedSpeakerVerifiedEvent`` with ``source: Literal["linkguard"]``,
...
:class:`NullLinkGuard` always reports ``UNAVAILABLE`` rather than ``PAUSED``.
"""
...
#: Outcomes after which live audio would overlap LinkGuard's own output.
...
@runtime_checkable
class LinkGuardAdapter(Protocol):
    """What a real LinkGuard integration must provide.

    Required semantics, all of which a real adapter has to honour:

    * **Idempotent.** Pausing an already-paused LinkGuard returns
      ``ALREADY_IN_STATE``, not an error, because a retry after a dropped
      response must be safe.
    * **Acknowledged, not fired.** ``pause`` must wait for LinkGuard to confirm,
...
class NullLinkGuard:
    ...
    ``UNAVAILABLE`` so that any code path treating "LinkGuard was paused" as
    ...
    def pause(self, *, session_id: int, timeout_seconds: float) -> GuardResult:
        return GuardResult(GuardOutcome.UNAVAILABLE, "no LinkGuard integration is configured")
...
def describe_overlap_risk(result: GuardResult) -> str:
    ...
    if not result.overlap_risk:
        return ""
    return (
        "LinkGuard was not confirmed paused, so this announcement may have "
