...
**Cooldown.** After EchoCast itself sends an enforcement command, that Store is
...
before EchoCast gives up and says so. A shop where somebody is determined to
...
#: How long a Store must be quiet before EchoCast acts on a mismatch. Long
...
#: How long a Store is left alone after EchoCast sends an enforcement command.
...
#: Attempts before EchoCast stops and says it has stopped. Three is enough to
...
class _StoreBookkeeping:
    ...
    #: Monotonic time EchoCast last sent an enforcement command.
...
class TargetEnforcementPolicy:
    ...
    def decide(self, *, store_id: int, target, actual_volume_percent,
        ...
        """Should EchoCast put this Store back to its target right now?
...
policy = TargetEnforcementPolicy(
    debounce_seconds=_tuned("ECHOCAST_TARGET_DEBOUNCE_SECONDS", DEBOUNCE_SECONDS),
    cooldown_seconds=_tuned("ECHOCAST_TARGET_COOLDOWN_SECONDS", COOLDOWN_SECONDS),
    max_attempts=int(_tuned("ECHOCAST_TARGET_MAX_ATTEMPTS", MAX_ATTEMPTS)),
    post_broadcast_seconds=_tuned("ECHOCAST_TARGET_POST_BROADCAST_SECONDS",
