# Running the SpeakLink Receiver automatically — Task Scheduler runbook
...
- A fresh Receiver package exists and passed
  `SPEAKLINK_RECEIVER_PACKAGE_VERIFIED`. Never use
  `artifacts\SpeakLinkReceiver-1.0.0` — it is marked `STALE-DO-NOT-DEPLOY`.
- This computer has already enrolled a Device
  (`SpeakLinkReceiver.exe enrol …`) and `status` shows a sealed credential.
...
Expect to end with `SPEAKLINK_RECEIVER_TASK_INSTALLED`.
...
```powershell
.\scripts\Test-SpeakLinkReceiverLanPilot.ps1 -IncludeLiveInstanceCheck `
    -LiveCredentialPath "$env:LOCALAPPDATA\SpeakLink\receiver\device-credential.bin"
```
...
Ends with `SPEAKLINK_RECEIVER_TASK_REMOVED`, or `SPEAKLINK_RECEIVER_TASK_ABSENT`
if there was nothing there. Running it twice is safe.

It refuses to remove a task whose action does not point at an
`SpeakLinkReceiver` executable — the task name is a parameter, and a typo could
...
- Any loudspeaker, amplifier, LinkGuard or `SPEAKER_VERIFIED` result. Every
