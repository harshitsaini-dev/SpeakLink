# SpeakLink One-Store Windows Output-Device Test Runbook
...
> LinkGuard acoustic detection, which does not exist yet.
...
$env:SPEAKLINK_AUDIO_OUTPUT_DEVICE = 'index:18@Headphones ()'   # your verified selector
```
...
| --- | --- |
| `SPEAKLINK_AUDIO_OUTPUT_DEVICE is not set` | Hardware mode needs an explicit selector. There is no default. |
...
```powershell
Remove-Item Env:SPEAKLINK_AUDIO_SINK_MODE     -ErrorAction SilentlyContinue
Remove-Item Env:SPEAKLINK_AUDIO_OUTPUT_DEVICE -ErrorAction SilentlyContinue
```

With no sink mode set, the pilot is back to `null` and cannot open any device.
The automated smoke (`.\scripts\Run-SpeakLinkAudioSmoke.ps1`) always uses the
...
`SPEAKER_VERIFIED` will only become possible when LinkGuard acoustic detection
